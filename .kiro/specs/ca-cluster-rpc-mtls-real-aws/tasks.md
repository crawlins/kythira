...
- [ ] 1. Extract `tests/aws_real_ec2_test_support.hpp`
...
- [ ] 2. Route `aws_quorum_manager_real_ec2_test.cpp` through the shared header
...
- [ ] 3. Add cost tracking + signal cleanup to `ca_cluster_node_real_ec2_test.cpp`
...
- [ ] 4. New `tests/ca_cluster_node_rpc_tls_real_ec2_test.cpp` — fixture and helpers
...
- [ ] 5. `bootstrap_and_cutover_survives_bootstrap_credential_deletion`
...
- [ ] 6. `staggered_third_node_join_maintains_connectivity`
...
- [ ] 7. `restarted_node_rejoins_without_bootstrap_credential`
...
- [ ] 8. `security_group_isolation_during_cutover_recovers`
...
- [ ] 9. Register `ca_cluster_node_rpc_tls_real_ec2_test` in `tests/CMakeLists.txt`
...
- These tests require live AWS credentials and `KYTHIRA_EC2_TEST_AMI`
  ...
  MUST be run manually against a real AWS account before considering this
  plan's tasks complete, per Requirement 8.4.
