...
installed", while every one of the file's 67 checkboxes stayed unticked. Both
halves were misleading, in opposite directions:

- **CI *is* such a machine, and always has been.** `grpc` is an
  *unconditional* top-level dependency in `vcpkg.json` — not a feature — so
  `find_package(gRPC CONFIG)` succeeds on every default build and
  `raft_grpc_transport` is always registered. Nothing in
  `.github/workflows/ci.yml` mentions gRPC by name, which is precisely why
  this went unnoticed for so long: the coverage was real but invisible to
  anyone grepping the workflow.
- **The evidence.** All three gRPC binaries —
  `grpc_transport_conversion_property_test`,
  `grpc_transport_integration_test` and `grpc_transport_example_test` —
  appear by name in the JUnit artifacts of run
  [30947491385](https://github.com/crawlins/kythira/actions/runs/30947491385)
  (commit `1a52e1f`, August 4, 2026) and pass on all four `Build & Test`
  legs (`g++-13`/`clang++-18` × x64/arm64), in a 403/403, 0-failure,
  0-skipped suite. That settles Task **1.5** (the generated `raft.pb.h`/
...
Implement `grpc_client<Types>`/`grpc_server<Types>` — a fourth
`network_client_type`/`network_server_type` pairing alongside cpp-httplib,
Boost.Beast and Proxygen, and the first that owns its own framing. Unlike the
serializer-only JSON/CBOR/protobuf/Ion path used over `tcp_rpc`/`tls_tcp_rpc`,
gRPC supplies HTTP/2 framing and codegen itself, so this is a transport
pairing rather than a new `serializer_type`. Deliberately independent of
`.kiro/specs/protobuf-rpc-serializer/`: distinct `.proto` packages, and
  ...
  "waves": [
    {
      "wave": 1,
      "tasks": [1],
      "description": "raft.proto plus vcpkg/Kconfig/CMake codegen — every later task assumes the generated headers exist, so nothing else can start first"
    },
    {
      "wave": 2,
      "tasks": [2, 3],
      "description": "Exception types and message conversion, and the grpc_transport_types concept plus config structs — both depend only on generated code, and are independent of each other"
    },
    {
      "wave": 3,
      "tasks": [4, 5],
      "description": "grpc_client and grpc_server core RPCs — depend on waves 1-2; independent of each other, though the integration tests in wave 6 need both"
    },
    {
      "wave": 4,
      "tasks": [6],
      "description": "TLS/mTLS groundwork — needs a working client and server to secure"
    },
    {
      "wave": 5,
      "tasks": [7, 8, 9],
      "description": "Optional extension services (RequestPreVote, ClusterJoin/ClusterLeave, fetch_log_entries) — mutually independent, each depends on wave 3"
    },
    {
      "wave": 6,
      "tasks": [10, 11],
      "description": "Property-based tests and unit/integration tests"
    },
    {
      "wave": 7,
      "tasks": [12],
    ...
    },
    {
      "wave": 8,
      "tasks": [13],
      "description": "Final validation — the only wave that requires a real CI run rather than local work"
    }
  ]
}
```
...
- [x] 1. Define `raft.proto` and generated-code build integration
  - [x] 1.1 Write `proto/raft.proto`
  ...
  - [x] 1.5 Verify generated code compiles standalone
...
- [x] 2. Implement exception types and message conversion
  - [x] 2.1 Create `include/raft/grpc_exceptions.hpp`
  ...
  - [x] 2.2 Create `include/raft/grpc_message_conversion.hpp`
...
- [x] 3. Implement `grpc_transport_types` concept and configuration structures
  - [x] 3.1 Define `grpc_transport_types` concept in `include/raft/grpc_transport.hpp`
  ...
  - [x] 3.2 Provide an example `grpc_transport_types` implementation
    - `kythira::Future`-backed example analogous to `http_transport_types`
    - _Requirements: 14.4_

  - [x] 3.3 Define `grpc_client_config`/`grpc_server_config`
...
- [x] 4. Implement `grpc_client<Types>` — core RPCs
  - [x] 4.1 Class skeleton
  ...
  - [x] 4.2 Implement `get_or_create_channel` (node-ID-keyed and address-keyed
        overloads)
  ...
  - [x] 4.3 Implement `build_channel_credentials` (TLS)
  ...
  - [x] 4.4 Implement generic `call_unary` helper
  ...
  - [x] 4.5 Implement `send_request_vote`/`send_append_entries`/`send_install_snapshot`
  ...
  - [x] 4.6 `static_assert(network_client<grpc_client<...>>)`
    - _Requirements: 1.1_

- [x] 5. Implement `grpc_server<Types>` — core RPCs
  - [x] 5.1 Class skeleton
    - Template with `Types` parameter, `requires grpc_transport_types<Types>`
    - Constructor accepting `bind_address`, `bind_port`, `config`, `metrics`
    - `_handlers`, `_server`, `_running`, `_mutex`
    - _Requirements: 1.2, 12.4, 14.1_

  - [x] 5.2 Implement `register_request_vote_handler`/`register_append_entries_handler`/
        `register_install_snapshot_handler`
    - _Requirements: 6.1_

  - [x] 5.3 Implement `RaftService::CallbackService` override
  ...
  - [x] 5.4 Implement `build_server_credentials` (TLS)
  ...
  - [x] 5.5 Implement `start()`/`stop()`/`is_running()`
  ...
  - [x] 5.6 Metrics emission (server)
    - Count/latency/size/error metrics per call; lifecycle metrics on start/stop
    - _Requirements: 13.1, 13.3, 13.4, 13.5_

  - [x] 5.7 `static_assert(network_server<grpc_server<...>>)`
    - _Requirements: 1.2_

- [x] 6. TLS/mTLS integration testing groundwork
  - [x] 6.1 Add a test helper issuing short-lived client/server certificates via
...
- [x] 7. Implement optional extension: RequestPreVote
  - [x] 7.1 Client: `send_request_pre_vote` against `RaftElectionExtensionService::Stub`
    - _Requirements: 16.1, 16.3_
  - [x] 7.2 Server: `register_request_pre_vote_handler` +
  ...
  - [x] 7.3 `static_assert`s for `network_client_with_pre_vote`/
        `network_server_with_pre_vote`
    - _Requirements: 16.1, 16.2_

- [x] 8. Implement optional extension: ClusterJoin/ClusterLeave bootstrap
  - [x] 8.1 Client: `send_cluster_join_request`/`send_cluster_leave_request` against
        `RaftBootstrapService::Stub`, addressed by contact address (not node ID) via
        `get_or_create_channel(const std::string&)`
    - _Requirements: 15.1, 15.3_
  - [x] 8.2 Server: `register_cluster_join_handler`/`register_cluster_leave_handler` +
        `RaftBootstrapService::CallbackService` override
    - _Requirements: 15.2_
  - [x] 8.3 `PeerInfo` redirect-hint round-trip
    - _Requirements: 15.4_
  - [x] 8.4 `static_assert`s for `network_client_with_cluster_join/_leave`/
        `network_server_with_cluster_join/_leave`
    - _Requirements: 15.1, 15.2_

- [x] 9. Implement optional extension: peer-to-peer `fetch_log_entries`
  - [x] 9.1 Client: `send_fetch_log_entries` against `RaftPeerReplicationService::Stub`
    - _Requirements: 17.1_
  - [x] 9.2 Server: `register_fetch_log_entries_handler` +
        `RaftPeerReplicationService::CallbackService` override
    - _Requirements: 17.2_
  - [x] 9.3 `LogEntry`/`entry_type` round-trip fidelity in `fetch_log_entries_response`
    - _Requirements: 17.3_
  - [x] 9.4 `static_assert`s for `network_client_with_log_fetch`/
...
- [x] 10. Property-based testing
  ...
  - [x] 10.6 **Property 6: Channel reuse for repeated calls to the same target**
    - **Validates: Requirement 12.3**
  - [x] 10.7 **Property 7: TLS is never silently downgraded**
    - **Validates: Requirements 9.1, 9.2, 9.6**

- [x] 11. Unit and integration testing
  - [x] 11.1 Unit tests for `grpc_client`
    ...
    - _Requirements: 1.1, 3.4, 3.5, 4.4, 4.5, 5.4, 5.5, 9.1, 9.3, 9.6, 10.1, 10.2, 12.1,
      12.2, 12.3, 13.2_
  - [x] 11.2 Unit tests for `grpc_server`
  ...
  - [x] 11.5 Integration test: graceful shutdown
    - `stop()` while calls are in flight completes them before returning
    - _Requirements: 7.2_

- [x] 12. Documentation and examples
  - [x] 12.1 Example program demonstrating gRPC transport usage
...
- **`grpc` is an unconditional `vcpkg.json` dependency, not a feature.**
  Unlike `ion` (opt-in via `--x-feature=ion`) or `gcp` (opt-in via
  `--x-feature=gcp`), gRPC and Protobuf are installed on every build, so
  `raft_grpc_transport` and its tests are always registered and always run.
  This is why `.github/workflows/ci.yml` contains no reference to gRPC and
  yet fully covers it — a genuine coverage/visibility mismatch that made this
  spec look unvalidated for longer than it was.
- **Independent of `protobuf-rpc-serializer`.** The two specs both use
  Protocol Buffers and are otherwise unrelated: distinct `.proto` packages,
  no shared code, and neither is a prerequisite for the other. In particular
  `doc/protobuf_serializer_performance_comparison.md` measures the
  serializer and does **not** satisfy Task 13.4's performance sanity pass for
  this transport.
- **Task 13's remaining items need a *negative* configuration**, which is
  what makes them awkward rather than expensive: 13.2 and 13.3 both require
  configuring with gRPC deliberately hidden
  (`-DCMAKE_DISABLE_FIND_PACKAGE_gRPC=ON`), which no current CI job does.
  The Folly-decoupling work established that this probe technique finds real
  breakage that inspection misses — see `doc/TODO.md`'s "Gap 2" entry, where
  a probe build surfaced six targets that had silently ignored the selected
  future backend for months.
