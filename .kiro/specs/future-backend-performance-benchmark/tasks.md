...
- [x] 0. Spike: confirm timing/scheduler-construction assumptions before writing scenario bodies
...
- [x] 1. Create `examples/future-backend-benchmark/benchmark_harness.hpp` skeleton
...
- [x] 2. Implement `folly_backend_traits::make_scheduler()` and `stdexec_backend_traits::make_scheduler()`
...
- [x] 3. Implement `bench_creation_resolution<Traits>` (Requirement 2.1)
...
- [x] 4. Implement `bench_same_thread_promise<Traits>` (Requirement 2.2)
  - `getFuture()` then `setValue` then `get()`, all on the calling thread
  - _Requirements: 2.2_

- [x] 5. Implement `bench_thenvalue_chain<Traits>` at configurable depth (Requirement 2.4)
...
- [x] 6. Implement `bench_thenerror<Traits>` (Requirement 2.5)
  - Build a future via `makeExceptionalFuture`, attach `thenError`, measure resolution
  - _Requirements: 2.5_

- [x] 7. Implement `bench_via_scheduler<Traits>` (Requirement 2.6)
...
- [x] 8. Implement `bench_collect_all<Traits>` at N = 10, 100, 1000 (Requirement 2.7)
  - Build N ready or near-simultaneously-resolving futures, measure `collectAll` fan-in
  - _Requirements: 2.7_

- [x] 9. Checkpoint — Phase 1 throughput scenarios complete
...
- [x] 10. Implement `bench_cross_thread_promise<Traits>` (Requirement 2.3)
...
- [x] 11. Implement `bench_collect_any<Traits>` (Requirement 2.8)
...
- [x] 12. Implement `bench_delay_within<Traits>` (Requirement 2.9)
...
- [x] 13. Document any scenario gap found during Phase 1/2 implementation (Requirement 2.10)
...
- [x] 14. Create `tests/future_backend_benchmark_test.cpp`
...
- [x] 15. Register the test target with CTest
...
- [x] 16. Verify CI behavior in both configurations
...
- [x] 17. Implement `examples/future-backend-benchmark/benchmark_report.cpp`
...
- [x] 18. Implement CSV/JSON artifact writing to `test_results/`
  - Timestamped filename following this project's existing `test_results/*_<timestamp>.*` naming
    convention
  - _Requirements: 5.2_

- [x] 19. Wire `future_backend_benchmark_report` into `examples/CMakeLists.txt`
...
- [x] 20. Write `doc/future_backend_performance_comparison.md`
...
- [x] 21. Run the full suite once end to end and capture reference numbers
