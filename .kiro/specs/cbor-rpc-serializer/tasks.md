...
This plan has not yet been implemented. `include/raft/cbor_serializer.hpp` does not exist
yet. Update this status line and the checkboxes below as work lands.
...
- [ ] 1. Implement CBOR codec primitives
  - [ ] 1.1 Create `include/raft/cbor_serializer.hpp` skeleton
  ...
  - [ ] 1.2 Implement `write_uint`, `write_bool`, `write_array_header`, `write_map_header`
    - Shortest RFC 8949 §3.1 encoding for every value (immediate 0-23, else 1/2/4/8-byte
      length-prefixed)
    - _Requirements: 8.1_
  - [ ] 1.3 Implement `write_byte_string`, `write_text_string`
    - Byte fields written as CBOR major type 2 directly (no base64 transcoding)
    - _Requirements: 2.3, 2.5_
  - [ ] 1.4 Implement `decode_cursor` and `read_major_type_and_info`
    - Bounds-checks `cur.pos` against `cur.end` before every advance/dereference
    - _Requirements: 6.1_
  - [ ] 1.5 Implement `read_uint`, `read_bool`, `read_array_header`, `read_map_header`
  ...
  - [ ] 1.6 Implement `read_byte_string`, `read_text_string`
    - Validate declared length against `cur.end - cur.pos` before `resize`/copy
    - _Requirements: 6.1_
  - [ ] 1.7 Implement narrowing-integer checks
    ...
    - _Requirements: 6.3_
  - [ ] 1.8 Implement `require_discriminant`
...
- [ ] 2. Implement `serialize`/`deserialize_*` for the core four RPCs
  - [ ] 2.1 `request_vote_request`/`response`, `request_pre_vote_request`/`response`
  ...
  - [ ] 2.2 `append_entries_request`/`response`
  ...
  - [ ] 2.3 `install_snapshot_request`/`response`
    - `data` written as a CBOR byte string, no base64 transcoding
    - _Requirements: 2.5, 2.6, 5.1_
  - [ ] 2.4 Round-trip each of the eight `deserialize_*` methods against its `serialize`
        counterpart
    - _Requirements: 2.7_

- [ ] 3. Implement `serialize`/`deserialize_*` for the bootstrap and peer2p2p
      extension RPCs
  - [ ] 3.1 `cluster_join_request`/`response`, `cluster_leave_request`/`response`
  ...
  - [ ] 3.2 `fetch_log_entries_request`/`response`
    - `entries()` reuses the same per-entry map shape as Task 2.2
    - _Requirements: 3.3, 3.4, 5.1_
  - [ ] 3.3 Round-trip each of the six `deserialize_*` methods against its `serialize`
        counterpart, including empty `entries()` and absent `redirect`
    - _Requirements: 3.5_

- [ ] 4. Implement generic dispatcher, `name()`, and concept conformance
  - [ ] 4.1 `template<typename T> deserialize<T>(const Data&) const -> T` dispatch table
    - One `if constexpr` branch per message type, mirroring
      `json_rpc_serializer::deserialize<T>`'s chain exactly
    - _Requirements: 2.8_
  - [ ] 4.2 `name()` returns `"cbor"`
    - _Requirements: 7.1_
  - [ ] 4.3 `static_assert(rpc_serializer<cbor_rpc_serializer<std::vector<std::byte>>,
        std::vector<std::byte>>)`
    - _Requirements: 1.1_
  - [ ] 4.4 Define `using cbor_serializer = cbor_rpc_serializer<std::vector<std::byte>>;`
    - _Requirements: 1.3_
  - [ ] 4.5 Confirm `coap_utils::get_content_format_for_serializer("cbor")` resolves to
        `coap_content_format::application_cbor` with no changes to that function
    - _Requirements: 7.2_

- [ ] 5. Build integration
  - [ ] 5.1 Confirm no `vcpkg.json` or root `Kconfig` changes are needed
    - `cbor_rpc_serializer` is a single new header with no external dependency
    - _Requirements: 8.1, 8.2_
  - [ ] 5.2 Register new test targets in `tests/CMakeLists.txt`
...
- [ ] 6. Unit tests
  - [ ] 6.1 `tests/cbor_serializer_concept_test.cpp`
...
- [ ] 7. Property-based tests
  - [ ] 7.1 `tests/cbor_serialization_property_test.cpp`
  ...
  - [ ] 7.2 **Property 3: Message-discriminant mismatch is detected**
  ...
  - [ ] 7.3 `tests/cbor_malformed_message_property_test.cpp`
  ...
  - [ ] 7.4 **Property 5: `name()` resolves to the CoAP CBOR content format**
    - For any instantiation of `cbor_rpc_serializer<Data>`
    - **Validates: Requirements 7.1, 7.2**
  - [ ] 7.5 Register all new property test targets in `tests/CMakeLists.txt`, each run at
        a minimum of 100 iterations and tagged
        `**Feature: cbor-rpc-serializer, Property {number}: {property_text}**`
    - _Requirements: 9.5_

- [ ] 8. Size-comparison test
  - [ ] 8.1 `tests/cbor_json_size_comparison_property_test.cpp`
  ...
  - [ ] 8.2 Document the comparison as a property test, not an exact-byte-count guarantee
    - _Requirements: 10.2_
...
- [ ] 9. Documentation
  ...
  - [ ] 9.2 Note the pre-existing HTTP transport `Content-Type` limitation
...
- [ ] 10. Final validation
  ...
  - [ ] 10.2 Confirm zero build-system impact
