...
- [x] 1. Define `raft_messages.proto` and generated-code build integration
  - [x] 1.1 Write `proto/raft_messages.proto`
  ...
  - [x] 1.2 Add `protobuf` to `vcpkg.json`
    - _Requirements: 9.4, 9.5_

  - [x] 1.3 Add `PROTOBUF_SERIALIZER` Kconfig symbol under a new `menu "Serialization"`
  ...
  - [x] 1.4 Wire `find_package(Protobuf)` and codegen into `CMakeLists.txt`
...
- [x] 2. Implement message-type tag and `NodeIdValue` helpers
  - [x] 2.1 Define `message_tag` enum (14 values) in `include/raft/protobuf_serializer.hpp`
    - _Requirements: 5.1_

  - [x] 2.2 Implement `encode`/`decode` private helpers
  ...
  - [x] 2.3 Implement `to_node_id_value<NodeId>`/`from_node_id_value<NodeId>`
...
- [x] 3. Implement `protobuf_rpc_serializer<Data>` — core RPC messages
  - [x] 3.1 Class skeleton with `requires serialized_data<Data>` constraint
    - _Requirements: 1.1, 7.1_

  - [x] 3.2 `to_proto`/`from_proto` for `LogEntry`/`EntryType`
    - _Requirements: 2.2, 6.5_

  - [x] 3.3 `serialize`/`deserialize_request_vote_request`/`deserialize_request_vote_response`
    - Using `NodeIdValue` for `candidate_id`
    - _Requirements: 3.3, 4.1, 4.2, 6.1_

  - [x] 3.4 `serialize`/`deserialize_request_pre_vote_request`/
        `deserialize_request_pre_vote_response`
    - _Requirements: 4.1, 4.2, 6.1_

  - [x] 3.5 `serialize`/`deserialize_append_entries_request`/
  ...
  - [x] 3.6 `serialize`/`deserialize_install_snapshot_request`/
        `deserialize_install_snapshot_response`
    - `data` passed through as raw `bytes` (no base64)
    - _Requirements: 4.1, 4.2, 6.1, 6.6_

  - [x] 3.7 `static_assert(rpc_serializer<protobuf_rpc_serializer<std::vector<std::byte>>,
        std::vector<std::byte>>)`
    - _Requirements: 1.1, 4.5_

- [x] 4. Implement `protobuf_rpc_serializer<Data>` — bootstrap and peer2peer extension
      messages
  - [x] 4.1 `PeerInfo` `to_proto`/`from_proto`, including absent-redirect handling
    - _Requirements: 2.3, 6.4_

  - [x] 4.2 `serialize`/`deserialize_cluster_join_request`/
        `deserialize_cluster_join_response`
    - _Requirements: 4.1, 4.2, 6.1, 6.4_

  - [x] 4.3 `serialize`/`deserialize_cluster_leave_request`/
        `deserialize_cluster_leave_response`
    - _Requirements: 4.1, 4.2, 6.1, 6.4_

  - [x] 4.4 `serialize`/`deserialize_fetch_log_entries_request`/
...
- [x] 5. Implement generic dispatcher and `name()`
  - [x] 5.1 `template<typename T> deserialize<T>(const Data&) const -> T` dispatch table
    - One `if constexpr` branch per message type, mirroring
      `json_rpc_serializer::deserialize<T>`
    - _Requirements: 4.3_

  - [x] 5.2 `name()` returns `"protobuf"`
    - _Requirements: 4.4_

- [x] 6. Unit and concept-conformance testing
  - [x] 6.1 `tests/protobuf_rpc_serializer_concept_test.cpp`
    - `static_assert` against `rpc_serializer`; smoke-test serialize/deserialize for
      `request_vote_request<>`; `name()` check
    - _Requirements: 10.1_

  - [x] 6.2 Register test target in `tests/CMakeLists.txt`, gated on `Protobuf_FOUND`
    - _Requirements: 9.1_

- [x] 7. Property-based testing
  - [x] 7.1 **Property 1: Round-trip fidelity for every message type**
  ...
  - [x] 7.2 **Property 2: Generic `NodeId` round-trips for both supported instantiations**
  ...
  - [x] 7.3 **Property 3: Wrong-tag payloads are always rejected**
    - Serialize as type `X`, attempt `deserialize_*` for every other type `Y`
    - **Validates: Requirements 5.1, 5.2, 5.6**
  - [x] 7.4 **Property 4: Malformed/random payloads are rejected at a high rate**
    - Random bytes, including 0-byte and 1-byte (tag-only) payloads
    - **Validates: Requirements 5.3, 5.4, 5.5**
  - [x] 7.5 **Property 5: Transparent drop-in for `json_rpc_serializer`**
...
- [x] 8. Benchmarking and documentation
  - [x] 8.1 Write a benchmark comparing `protobuf_rpc_serializer` vs. `json_rpc_serializer`
    - Payload size and serialize/deserialize latency across varying `entries()` counts
      and command sizes
    - _Requirements: 8.1_
  - [x] 8.2 Record results in `doc/protobuf_serializer_performance_comparison.md`
    - _Requirements: 8.2_
  - [x] 8.3 README updates
...
- [x] 9. Final validation
  - [x] 9.1 Run the full test suite with `PROTOBUF_SERIALIZER` enabled
    - All unit, property, and integration tests compile and pass under CTest
    - _Requirements: 10.1, 10.2, 10.3, 10.4_
  - [x] 9.2 Confirm graceful degradation with `PROTOBUF_SERIALIZER` unset/Protobuf absent
  ...
  - [x] 9.3 Confirm `KYTHIRA_KCONFIG_STRICT` failure path
    - Configuration fails loudly when `PROTOBUF_SERIALIZER` is selected but Protobuf is
      missing
    - _Requirements: 9.2_
  - [x] 9.4 Confirm coexistence with `GRPC_TRANSPORT`
