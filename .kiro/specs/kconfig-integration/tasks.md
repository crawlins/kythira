...
- [x] 1. Write `Kconfig` at the repository root
...
- [x] 2. Validate the `Kconfig` file with Kconfiglib directly (no CMake yet)
...
- [x] 3. Write `configs/ci_full_defconfig` and `configs/minimal_defconfig`
...
- [x] 4. Write `scripts/kconfig/genconfig.py`
...
- [x] 5. Pin the Kconfiglib dependency
  - Add `scripts/kconfig/requirements.txt` with a pinned `kconfiglib==`
    version
  - _Requirements: 8.2_

- [x] 6. Unit-verify genconfig.py against each defconfig from Task 3
...
- [x] 10. Rewire the Poco DNSSD block and the `AWS_ACM_PCA`
...
- [x] 11. Wire `CONFIG_COVERAGE` to `ENABLE_COVERAGE`
...
- [x] 12. Add `menuconfig`, `guiconfig`, `savedefconfig`, `kconfig-check`
  custom targets
...
- [x] 13. Zero-config regression check
...
- [x] 14. Strict-mode failure check
...
- [x] 16. `menuconfig` round-trip check
