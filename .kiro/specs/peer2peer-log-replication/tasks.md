...
Defaults to a no-op that preserves today's leader-only behavior exactly.
...
```json
{
  "waves": [
    ...
    },
    {
      "wave": 5,
      "tasks": [13, 14],
      "description": "Maintenance-thread wiring (needs 10-12) and metrics/logging (needs 10-12; independent of 13)"
    },
    {
      "wave": 6,
      "tasks": [15, 16, 17, 18, 19],
      "description": "Test suite — depends on all implementation waves; the five test tasks are independent of each other"
    }
  ]
}
```
...
- [ ] 1. Add `peer2peer_replicator` concept and `no_op_peer2peer_replicator`
  ...
  - `peer2peer_replicator<P, NodeId, Address, LogIndex>` concept requiring
    ...
    Future<void>` and `find_catch_up_source(from_index, to_index, timeout)
    -> Future<std::optional<peer_info<NodeId, Address>>>`
  - `no_op_peer2peer_replicator<NodeId, Address, LogIndex>`:
    `advertise_progress` resolves immediately; `find_catch_up_source` always
    resolves `std::nullopt`
...
- [ ] 2. Add `static_peer2peer_replicator` reference implementation
  ...
  - `advertise_progress` writes/overwrites this node's entry;
    `find_catch_up_source` returns the first peer whose gossiped
    `last_log_index >= from_index`
...
## Phase 2: `node<Types>` Wiring and Consistency-Check Extraction (Tasks 8–9)
...
- [ ] 8. Wire `peer2peer_replicator_type` into `node_config<Types>`/`node<Types>`
  ...
  - Verify: every existing test file constructing `node<Types>`/
    ...
    gate — full check happens in Task 19, but this task's own build must not
...
## Phase 3: Catch-Up Logic (Tasks 10–14)

- [ ] 10. Implement `handle_fetch_log_entries()` (responder side)
...
- [ ] 11. Implement `maybe_gossip_progress()`
...
- [ ] 12. Implement `maybe_catch_up_from_peer()`
...
- [ ] 13. Wire `maybe_gossip_progress()`/`maybe_catch_up_from_peer()` into the
  ...
  - Verify: a running simulator-based node calls both on schedule
    (observable via the debug logging added in Task 14) without disrupting
    existing heartbeat/election timing
  - _Requirements: 3.1, 4.1_

- [ ] 14. Add metrics and logging
  ...
  - Verify: log output inspected manually during Task 12's unit tests shows
...
## Phase 4: Tests (Tasks 15–19)

- [ ] 15. Unit tests for concept, no-op/static implementations, and the
  ...
  - `tests/peer2peer_replication_unit_test.cpp` (new file): concept
    ...
    resolving as specified, `static_peer2peer_replicator` table behavior
...
- [ ] 16. Property test: new node catches up via a peer, not the leader
...
- [ ] 17. Property test: partition-reconnect catch-up via a peer
...
- [ ] 18. Safety property test: stale/incorrect source peer still converges
      correctly
...
- [ ] 19. Full regression and no-op-default equivalence
...
- `static_peer2peer_replicator` (Task 2) is intentionally the *only*
  ...
  follow-on spec, mirroring `node-bootstrap` → `dns-peer-discovery`.
- Task 12's single-flight guard and Task 8's constructor wiring touch the
  same class (`node<Types>`) but different concerns (state vs. construction)
  — implement Task 8 first so Task 12 has a member to guard.
