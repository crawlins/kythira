...
The centerpiece is a `peer_finder` concept that abstracts peer discovery, a new
`ClusterJoin` RPC pair for the initial membership request, and a bootstrap flow
wired into `node::start()`. All additions are opt-in: the default `no_op_peer_finder`
...
run_bootstrap():
  peer_finder.find_peers(timeout).get()
  │
...
### 1. `include/raft/peer_finder.hpp` — new file
...
#### `no_op_peer_finder<NodeId, Address>`

```cpp
template<typename NodeId, typename Address>
class no_op_peer_finder {
public:
...
#### `static_peer_finder<NodeId, Address>`

```cpp
template<typename NodeId, typename Address>
class static_peer_finder {
public:
    using node_id_type  = NodeId;
    using address_type  = Address;

    explicit static_peer_finder(std::vector<peer_info<NodeId, Address>> peers)
...
typename T::peer_finder_type;
requires peer_finder<typename T::peer_finder_type,
...
using peer_finder_type  = no_op_peer_finder<node_id_type, address_type>;
```
...
using peer_finder_type          = typename Types::peer_finder_type;
...
node(node_id_type node_id,
     ...
     peer_finder_type peer_finder = peer_finder_type{})  // new, optional
```
...
```cpp
// In network_client concept (optional — only required when peer_finder is used)
...
1. Waits up to `_config.peer_finder_isolation_timeout()` for a notification
...
3. If timeout elapses: calls `_peer_finder.find_peers(...).get()`, passes each
...
A new `raft_configuration` field `peer_finder_isolation_timeout` defaults to
...
1. Calls `_peer_finder.find_peers(_config.bootstrap_peer_find_timeout())`
...
auto node<Types>::start() -> void {
    ...
    if (is_fresh_node()) {
        run_bootstrap();       // no-op if peer_finder returns empty
    }
...
### 6. `include/raft/coap_transport.hpp` — `coap_multicast_peer_finder`

A thin adaptor class in `include/raft/coap_transport.hpp`:

```cpp
class coap_multicast_peer_finder {
public:
    using node_id_type  = std::string;
    using address_type  = std::string;

    explicit coap_multicast_peer_finder(
...
With `no_op_peer_finder` (the default), `run_bootstrap()` calls `find_peers()`
...
`_voted_for`. It cannot form a quorum alone because `no_op_peer_finder` produces
...
simulator. The `peer_finder` used in tests is `static_peer_finder`.

- `node_bootstrap_unit_test.cpp` — unit tests for `peer_finder` concept
  satisfaction, `no_op_peer_finder`, `static_peer_finder`, `is_fresh_node()`,
...
- `node_bootstrap_join_property_test.cpp` — property test: single-node cluster
  running; fresh node with `static_peer_finder` pointing at it starts; after
