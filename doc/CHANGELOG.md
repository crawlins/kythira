...
- **Task 0's last three providers closed live: Azure Blob, GCS and OCI Object Storage** (`.kiro/specs/cloud-object-persistence/spike-notes.md` Findings 11-14, via three new probes in `scripts/object-store-probes/`). All three express **every** conditional primitive, conditional delete included — which closes the OCI cell design.md had marked unconfirmed and makes **four of the five providers able to carry `fencing_mode::compare_and_swap`; only Alibaba OSS cannot**. Every cell that needed a live service is now closed for all five providers, none by analogy: list-after-write ran 3 × 25 objects with an immediate LIST on each, complete every round.
  - **One provider answers two different statuses for the two preconditions.** Azure rejects `If-None-Match: *` with **409 `BlobAlreadyExists`** and `If-Match` with **412 `ConditionNotMet`** — both meaning "you lost", both of which must latch a fence. Set against S3's **409 `ConditionalRequestConflict`**, which means "benign race, retry", the rule the OSS run produced gets stronger rather than merely repeated: **a client may never map a bare status; it must read the error code**. Requirement 13.4 said "map 412" and is corrected in place — mapping only 412 would have let a create-only collision, the exact case that catches a stale leader appending entries, pass as an ordinary error.
  - **Requirement 7's two halves come apart across providers.** The service-side checksum check exists on all five and is spelled five different ways (`BadDigest`, `InvalidDigest`, `Md5Mismatch`, `invalid`, `UnmatchedContentMD5`), but the **ETag is the content MD5 on only two of them** — S3 and OSS. Azure's is an opaque timestamp token, **OCI's is a UUID**, GCS's is an opaque token, and on those three the locally verifiable value is the returned `Content-MD5` / `opc-content-md5` / `md5Hash` metadata field instead. A client that assumed the S3 shape would compare an opaque token against an MD5 and fail every write.
  - **GCS's version token is a decimal generation carried as a query parameter**, not a hash in a header — the concrete justification for `object_version` being an opaque string the engine never inspects. An engine that assumed "quoted hash" would be wrong on GCS; one that assumed "counter" would be wrong everywhere else.
  - **Task 0.6 closed in favour of the recorded decision**: the Azure probe *is* the hand-rolled REST surface, and nothing about it was harder than documented, so no `azure-storage-blobs-cpp` dependency is needed. Three details the client must record because each is a plausible wrong guess: success is **201 on PUT and 202 on DELETE**, the machine-readable error code is the **`x-ms-error-code` response header**, and a bodiless PUT still needs `Content-Length: 0`. **Task 0.8 closed too**: the namespace is both configurable and resolvable (`GET /n/` returns it as a bare JSON string), so the OCI client should resolve-and-cache only when none is configured.
  - **Writing the OCI signer by hand found a real client obligation**: `oci_signing.hpp` signs `content-type: application/json` as a **constant**, because every existing caller is a control-plane API. Object Storage is a data plane with opaque bodies, so task 10's raw-bytes path must make the signed content type a parameter — signing `application/json` while sending `application/octet-stream` yields a 401 that says nothing about content types.
  - **Two probe defects, both caught by controls rather than by luck, and both worth more than the cells they nearly corrupted.** `headers.get("ETag")` returned nothing because **OCI spells the header `etag`**; the probe sent `if-match: ""`, the service answered 412 to a *correct*-ETag write, and the run read exactly like "OCI cannot do CAS" — the same finding Alibaba genuinely earned. The **negative control** caught it: `cas_verdict()` refuses to call CAS usable unless a correct precondition is *accepted*, so it printed "NOT USABLE — even a correct precondition was refused" instead of a false finding about a provider's fencing capability. Response headers are now case-insensitive for every provider. Separately, the checksum verdict trusted "any 4xx" and duly printed **"VERIFIED by the service (BucketNotFound)"** for a request refused for an unrelated reason; it now requires a checksum-specific error code and reports UNDECIDED otherwise, which is how OCI's actual code came to be recorded rather than assumed. **A status class is not a reason** — the effect-versus-status-code trap, one level up.
  - **An OCI environment observation, recorded as an observation and not as a cell**: this tenancy intermittently declines valid requests with `404 BucketNotFound` ("...or you are not authorized") against a bucket that plainly exists, at roughly 3-16% depending on the moment, across every request shape. It resembles the earlier case where a policy `where` clause declined a *different* principal's Object Storage requests, and it needs its own investigation with an `opc-request-id` against the audit log. No cell rests on a request that hit it: measurement requests are re-issued once past a decline, every re-issue prints, and the count is reported. It also forced a methodological fix — **list-after-write compares the listing against *acknowledged* writes, never against the number attempted**; the earlier run's "0 of 25 listed" was ten declined PUTs, and reading it as a lagging listing would have invented a consistency defect out of an authorization flake.

- **Task 0's two documentation cells closed, one of them with a negative result.** **0.2 (OCI durability-on-response wording): the sentence does not exist**, and that is now written down instead of being borrowed from the other four providers. Oracle publishes a steady-state redundancy statement and a read-side strong-consistency statement; **neither says when the redundancy exists relative to the PUT response**, which is the exact claim this engine's fsync-equivalence argument rests on. The N1 cell stays OPEN in the durability table but now reads "searched and not found, August 17, 2026" — a materially different and more useful state than an unexamined cell — and the operator documentation must carry the gap visibly. It blocks nothing in the client; a support ticket is the remaining avenue, recorded as such because a support answer is evidence of a different kind from published documentation.
  - **0.4 (single-PUT size limits): S3 5 GB, Alibaba OSS 5 GB, Azure Blob 5,000 MiB *for `x-ms-version` 2019-12-12 and later* (256 MiB before — a second reason the client pins a dated version), OCI 50 GiB, GCS 5 TiB.** Azure is the only one that names the failure mode: 413 Request Entity Too Large. **The default `max_object_bytes` is nevertheless 64 MiB, not the smallest limit**, and Requirement 7.3's "chosen from the smallest" is corrected to say why: the binding constraint is the engine's shape, not the service's. One retry, no multipart, no resumption, the mutex held for the whole round trip, ~2-3 s measured per round trip, and AWS's own advice to abandon single-PUT above 100 MB. The cap exists to turn "this deployment has outgrown a single-PUT persistence engine" into a loud error at the first snapshot that reaches it — not to predict the service's rejection.

- **Cloud object persistence, task 4: snapshot retention** (`.kiro/specs/cloud-object-persistence/`, `include/raft/object_store_persistence.hpp`). `object_persistence_options::snapshot_retention` keeps N snapshot generations, retained predecessors accumulating at `<prefix>/snapshots/<20-digit last_included_index>` under the same codec and the same padding rationale as the log keys. The point is narrow and stated as such: a snapshot written from corrupted in-memory state overwrites the single slot, and without a predecessor there is nothing left to go back to. It is **not backup** — retained copies share a bucket, a prefix, a credential and a blast radius with the thing they would be recovering from, which is why the disclaimer is a deliverable of the retention feature itself rather than a line in a document nobody reads.
  - **Retention counts generations including the live slot**, which is what makes the default of `1` mean today's behaviour with no special case in the arithmetic: no `<prefix>/snapshots/` object is written at all, `save_snapshot` stays exactly one PUT, **and it issues no LIST and no DELETE**. The conformance case asserts the request counts and not merely the key set, because a retention feature that quietly added one request per snapshot would pass a key-set assertion and still have changed every existing deployment's cost. `0` is rejected at construction rather than read as "keep none", which would delete the retained copy of the snapshot just written.
  - **The three-step order is the safety property**: retained copy, then `<prefix>/snapshot` — the commit point — then prune. A failure at the first leaves the previous state entirely intact, at the second an unreferenced retained copy that nothing reads, at the third extra copies costing storage. Three conformance cases inject a failure at each step (the commit-point one at retention 1, 2 and N) and assert a readable current snapshot survives all three.
  - **Pruning deliberately does not throw, and deliberately is not silent either.** It runs *after* the commit point, so reporting a failed DELETE as a failed `save_snapshot` would tell the caller its snapshot was lost when it was not — and in `node::install_snapshot` an exception from `save_snapshot` abandons the following term write and the RPC reply over what is a garbage-collection error. Instead the failure is recorded in `last_prune_error()`, the undeleted index stays in the retained set, and the next `save_snapshot` prunes it again: observable and self-healing, which is what "costs storage and nothing else" actually requires. Requirement 8.2 now says so explicitly rather than leaving the error-reporting half to the implementer.
  - **Retained indices are noted at construction from the listing the load path already performs**, so pruning needs no LIST of its own and steady state stays at the two PUTs the design's request model budgets. The load path still **never GETs a retained copy** — recovery is `<prefix>/snapshot` and nothing else — so a corrupt retained copy cannot break startup, and a case seeds one to prove it (and asserts the object was never even fetched).
  - **An unparseable key under `<prefix>/snapshots/` is neither pruned nor fatal, which is the opposite of the same shape under `<prefix>/log/`.** The asymmetry is the point: a log key that cannot be ordered breaks recovery and is therefore corruption, whereas a retained copy is never read by the load path at all, so the safe reading of an unrecognized one is "somebody else's object". An operator's note and an unpadded index under `snapshots/` both survive a full save-and-prune cycle.
  - **The new cases were checked against a mutant, not just against themselves**: skipping the prune and writing a retained copy at retention 1 fails 13 cases — including `a_default_engine_writes_exactly_the_documented_key_set` and `snapshot_overwrites_the_single_slot`, which predate retention. A test suite that passes both before and after the behaviour it claims to pin is evidence of nothing, and this is the cheap way to find that out.

### What Changed (August 16, 2026)

- **Alibaba OSS cannot be fenced, measured live — and the hoisted engine re-verified against the real service.** Task 0's decisive cell (`.kiro/specs/cloud-object-persistence/spike-notes.md`, new) is closed against live OSS in `ap-southeast-1`: **`If-Match` on PutObject returns `400 NotImplemented` for a *current* ETag as well as a stale one**, so there is no ETag-predicated write to build a compare-and-swap fence on. Requirement 9.8 therefore fires exactly as the spec anticipated — `alibaba_oss_client` will satisfy `key_object_store` and **not** `conditional_key_object_store`, making `compare_and_swap` a **compile error** for the OSS engine rather than a runtime degradation. The spec named this outcome in advance, against the provider its own reference implementation came from, and forbade working around it; the measurement is what turns that from caution into fact.
  - **The 409 collision is the sharp new finding.** OSS's create-only header (`x-oss-forbid-overwrite: true`) works and rejects with **409 `FileAlreadyExists`** — not 412 — while S3 uses **409 `ConditionalRequestConflict`** for the *opposite* meaning, a benign race the caller should retry. The same status code means "you definitively lost" on one provider and "try again" on another, so **no client may map a bare 409 to either meaning**; the error code must be read. A generic "409 → retry" rule would turn OSS's only working precondition into a loop that eventually overwrites.
  - **OSS's conditional DELETE is accepted and silently ignored** — a stale `If-Match` returns 204 and the object is gone. Not a 400 like the unsupported PUT preconditions, and not a 412. The design already keeps every DELETE unconditional (Requirement 9.3's stated residual), so this costs nothing; what it changes is the justification, from "buys little" to "is not available on at least one provider".
  - **A methodological trap, recorded because it nearly produced a false confirmation**: the probe's own verdict line asked "did the stale write change the bytes?", saw that it had not, and printed *"CAS works"*. The write had in fact been refused as unimplemented — no precondition was ever evaluated. A live probe whose success criterion is the *effect* rather than the *status code* cannot tell a working fence from a rejected request, and would have written a fence that does not exist into the design.
  - **Confirmed the same run**: `Content-MD5` is verified end to end (`400 InvalidDigest` on mismatch) and the ETag is the uppercase MD5 hex of single-part content, so both halves of Requirement 7 are available on OSS; list-after-write returned all 25 objects on each of three immediate rounds, moving OSS's N3 cell from OPEN to **empirically consistent** — deliberately weaker language than "guaranteed", since the vendor still publishes no listing-specific statement. Bucket versioning is unset on the CI bucket, so the documented "forbid-overwrite is silently ignored when versioning is enabled" hazard was dormant rather than absent — it stays an operator-documentation obligation, since it is a setting the engine neither controls nor sees.
  - **Task 19's Alibaba half, done early**: `alibaba_oss_persistence_real_test` ran **4/4 green against the live bucket after the hoist**, so the live-verified claim now belongs to `object_store_persistence_engine<alibaba_oss_client>` rather than to the engine that was replaced. Case wall times (8.6 s / 29.5 s / 14.5 s / 5.2 s) are recorded as evidence with their provenance intact — cross-ocean, fixture cleanup included, **not** per-operation latency and not quotable as production numbers; Requirement 4.5's measured p50/p99 is still owed.
  - Probes were run with a **standalone OSS V4 signer written from the canonical form documented in `alibaba_oss_client.hpp`**, deliberately not calling the C++ client: the client cannot send conditional headers (that is what the spike decides), and an independent implementation is a second witness to the signing scheme — it authenticated first try, which is itself a confirmation. 85 objects created, 85 deleted, 0 residual. Now kept as `scripts/object-store-probes/` (README + `probe_alibaba_oss.py`), so the other four providers' cells are a sibling script rather than a rewrite — with the effect-versus-status-code lesson encoded in a `classify_precondition()` helper that distinguishes *enforced* (409/412) from *unsupported* (400 `NotImplemented`) from *silently ignored* (2xx, object unchanged), and a re-run from the promoted location reproducing every verdict.
  - **AWS S3's task-0 cells closed live, and they went the opposite way to Alibaba's** (spike-notes.md Finding 10, via a new SigV4 sibling probe `scripts/object-store-probes/probe_aws_s3.py` against the freshly provisioned bucket): create-only `If-None-Match: *` → **412 `PreconditionFailed`** with the object unchanged, `If-Match` overwrite CAS works (current ETag 200, stale 412), and **conditional DELETE works** — a stale `If-Match` is refused *and the object survives*, which is exactly the negative control Alibaba failed. That closes the `if-match`-on-DeleteObject cell design.md had marked OPEN and makes **S3 the first provider that could carry `fencing_mode::compare_and_swap`**. `Content-MD5` is verified end to end and the ETag is the MD5 hex of single-part content, so both halves of Requirement 7 apply; list-after-write returned all 25 objects on each of three immediate rounds.
  - **The 409 cell came out empty, and is recorded as empty rather than as absent.** Racing 8, then 16, then 32 concurrent create-only PUTs at one key produced exactly one winner per round and **53 losers, every one of them 412** — not a single 409 `ConditionalRequestConflict`. AWS documents 409 for two conditional requests racing, and 56 requests from one host is a thin sample of a distributed coordination path, so the design's rule is unchanged and now says explicitly that it rests on documentation: **the S3 client maps 412 and 409 separately, and the unit case pinning that stays mandatory.** Writing down "not observed in 56 requests" as "cannot happen" is precisely how a rare-but-real path becomes an unrecoverable fence latch in production.
  - **Three cross-provider spellings now differ measurably**, none of which the engine sees (`object_version` is opaque) and every one of which a client must get right: rejection code (S3 **412 PreconditionFailed** vs OSS **409 FileAlreadyExists**), checksum-mismatch code (S3 **BadDigest** vs OSS **InvalidDigest**), and ETag case (S3 lowercase, OSS uppercase — so a local ETag verification must compare case-insensitively).
  - **The effect-versus-status-code trap recurred a third time — inside the helper written to prevent it.** `classify_precondition()`'s S3 debut labelled a plainly successful conditional write "SILENTLY IGNORED", because the call site passed the *post-write* content as "what would be stored if the write did not happen", so the helper compared the landed bytes against themselves. The status-code-keyed verdict line was right throughout; only the per-write label was wrong. The parameter is now named `bytes_if_write_did_not_happen` and both call sites pass the previous content. The generalizable lesson is not "be careful" but that **a check whose two inputs can be transposed without a type error will eventually be transposed**.
  - **The three missing buckets are provisioned** (task 17's bucket half, spike-notes.md Finding 8), each by an idempotent `provision-object-persistence-*` script in `scripts/ci-cloud-credentials/{aws,azure,gcp}/`, each run twice and each verified with a real write/read/delete round trip under the `kythira-real-test/` prefix the shipped Alibaba suite already uses: **S3** `kythira-ci-827617851594` (us-east-1, public access blocked, SSE-S3, 7-day lifecycle on the test prefix); **Azure Blob** account `kythirarealtestobj` container `kythira-raft` (eastus, **Standard_ZRS**, TLS 1.2, no public blob access); **GCS** `kythira-ci-prefab-sky-500619-s9` (us-central1, uniform bucket-level access, public access prevention enforced, **soft delete off**, 7-day lifecycle). OCI and Alibaba reuse their existing buckets, so all five providers now have somewhere to write. Two settings are decisions rather than defaults, and the scripts say why in their headers: **ZRS** because it is the only Azure redundancy mode documented as writing synchronously to all three zone replicas before returning success — the exact thing design.md's N1 row needs — and **soft delete off** on GCS because its 7-day default bills deleted objects for a week on a bucket whose whole workload is create-and-delete.
  - **Two Azure provisioning traps, both of which report something misleading, are now handled in the script and recorded** (Finding 9). A subscription **Owner can create a storage account and a container and still not write a single blob** — containers are management-plane resources, so `container create` *succeeded* while every blob operation returned "You do not have the required permissions"; container creation is therefore not the evidence it appears to be, and the script now proves the data plane with a real round-trip probe (with `--grant-caller-data-role` to assign `Storage Blob Data Contributor`, which took ~45 s to propagate). And a subscription that has never held a storage account has **`Microsoft.Storage` unregistered**, which makes every storage call fail with `SubscriptionNotFound` — a message that reads like the subscription is gone.
  - **A pipeline hid a failure, and nearly published it as a success**: the first Azure smoke test piped `az` into `tail` and tested `&&`, so it read *`tail`'s* exit status and printed "uploaded" for a command that had failed outright. Same shape as the effect-versus-status-code trap above, in a different costume — the lesson generalizes to "check the exit status of the thing you actually ran".
  - **Credential locations recorded** (spike-notes.md), because "AWS is unreachable" turned out to be false and cost a cell: all five providers authenticate from the development host, but **AWS's working credentials are in the `personal` CLI profile**, not `default`/`clark`, which are expired. That profile resolves to the account **root** ARN — acceptable for a read-only spike, and explicitly **not** what task 17's least-privilege provisioning should use or assume.

### What Changed (August 15, 2026)

- **Cloud key-object persistence, tasks 1-3: the seam, the generic engine, and the Alibaba engine as an instantiation** (`.kiro/specs/cloud-object-persistence/`). New `include/raft/key_object_store.hpp` (five operations plus a `conditional_key_object_store` refinement, compiled unconditionally and naming no provider), new `include/raft/object_store_persistence.hpp` (`object_store_persistence_engine<Store>` — `alibaba_oss_persistence_engine`'s body, **hoisted rather than rewritten**), and `alibaba_oss_persistence_engine` redefined as a thin derived type over `alibaba_oss_client` keeping its `{alibaba_client_config, bucket, prefix}` constructor. **These three tasks add no capability on purpose**: they exist so that retention, fencing and backup are written once instead of five times, and so a default-configured bucket stays byte-identical to what ships.
  - **The hoist's verification bar was the point, and it was met as written**: `alibaba_oss_persistence_unit_test` (41 cases), `alibaba_oss_persistence_mock_test` and `alibaba_oss_persistence_real_test` compile and pass with **zero source changes** — not one assertion moved. The only test lines edited anywhere are two in `alibaba_oss_client_unit_test.cpp` (`.value()` → `.value().body`), the mechanical consequence of `get_object` now returning a `get_result`. Editing a test into agreement is how a refactor of a live-verified component quietly discards the evidence that made it valuable; refusing to is the whole argument for the bar.
  - **A shared conformance suite is the mechanism that makes five engines cost one engine's worth of review** (`tests/object_store_conformance.hpp`): one test body, 43 cases, instantiated per store in one line. It runs here against `tests/mock_object_store.hpp` — in-memory, satisfying both concepts, with a request log and injectable failures — and covers round-trip and reload-survival per field, binary commands with embedded nulls and high bytes, the 20-digit padding boundary (9 → 10), truncation and compaction, durability ordering read from the request log, PUT-failure-leaves-the-mirror-unchanged, and every corrupt-object-fails-construction case. Two properties get cases of their own because they are how a "small, harmless" change does damage silently: the **exact key set** a default-options engine produces (a new feature writing one extra object would change the on-bucket format for every existing deployment), and a **foreign object surviving a full truncate/compact cycle** (the engine deletes only what it can prove it wrote).
  - **The mock store is not a mock HTTP server, deliberately.** The per-provider mock servers exist to prove a *client* speaks its wire protocol, signatures included; the engine has no wire protocol, so testing it through one would only add a way for a transport detail to be mistaken for engine behaviour. What the in-memory store can do that a socket cannot is **lag a listing** — and that case is pinned as a test asserting the engine recovers a *silently short log*, because that is the one consistency failure the engine cannot detect at runtime and closing it is a provider obligation, not an engine one.
  - **Fault points now exist at both altitudes**: provider-independent `raft/objstore/{put,get,delete,list}_object` on the generic engine, and the existing `raft/alibaba/oss/*` names moved onto `alibaba_oss_client` itself, so no chaos configuration breaks. The existing fault-injection cases passing unchanged is the evidence the move is invisible.
  - **One recorded deviation from the design sketch**: the concept's requires-expression names `const std::string&` for `bucket`/`key` rather than `std::string_view`. It is the more permissive of the two — a `string_view`-parameter client still satisfies it, while the reverse fails because `std::string`'s converting constructor from `string_view` is explicit — and it is what lets the live-verified OSS client keep its signatures.
  - **`object_persistence_options` ships partial, on purpose**: `write_retries` only. Retention, fencing, checksum verification and the size cap arrive **with** the behaviour that honours them (tasks 4-6). A fencing knob that is accepted and ignored is precisely the silent degradation Requirement 9.8 exists to forbid, and `max_object_bytes`'s default cannot be chosen before task 0 measures the per-provider limits.
  - **Task 0, the spike, is blocked on credentials rather than skipped**: no Alibaba CLI profile exists on the development host any more and its AWS credentials are expired, so the conditional-write matrix — including whether OSS supports ETag CAS at all, which decides whether Alibaba can be fenced — cannot be closed from there. Nothing implemented so far depends on a cell it closes; fencing and integrity do, which is why they were not started. The Alibaba real suite likewise builds but has not been re-run live, so **the live-verified claim still belongs to the pre-hoist engine** until task 19 runs.

### What Changed (August 14, 2026)

- **Cloud key-object persistence spec authored** (`.kiro/specs/cloud-object-persistence/` — requirements/design/tasks; **no implementation**), closing doc/TODO.md's "write the spec" mandate: one `kythira::persistence_engine` per implemented provider over AWS S3, Azure Blob Storage, GCP Cloud Storage, OCI Object Storage and the Alibaba OSS engine that already ships. **Load-bearing decision: one engine, five stores** — the Raft-correctness body (layout, 20-digit padding, in-memory mirror, mirror-after-acknowledgement ordering, parse-or-throw load, retry, retention, fencing) is written once as `object_store_persistence_engine<Store>` over a new `kythira::key_object_store` concept, and each provider contributes only a client. That is a **deliberate departure** from the independent-sibling shape of the quorum managers and certificate providers, argued on the grounds that compute-fleet APIs differ semantically (ESS vs ASG vs MIG vs instance pool: different capacity models, different liveness signals, additive vs whole-map tagging) while object stores do not. `alibaba_oss_persistence_engine` becomes an instantiation keeping its name, constructor and tests — and the spec's verification bar for that hoist is that its existing 41 unit cases, its mock suite and its **live-verified** real suite all pass *unmodified*.
  - **The synchronous-flush requirement is confronted, not softened.** No batching, no write-behind, no relaxed-durability mode, no async flush — Requirement 4.7 records `save_hard_state(term, vote)` as the attractive optimization it refuses, because it changes the concept every engine implements. The cost is instead made computable: one election round is **four sequential durable writes** (candidate term + self-vote, then each follower's term + vote before replying), so `election_timeout_min ≥ 4 × p99(PUT) + rpc_rtt`, and sustained append throughput is bounded by one PUT round trip per entry per node. The only measurement this project has is **~2-3 s per object round trip** to `ap-southeast-1` from a developer machine (Alibaba spike Finding 7 — 9.6 s for term+vote, 34 s for 12 appends), which the spec forbids quoting as a production number: it is geography, and in-region measurement is a deliverable, not a hope. Worked out: that figure gives a ≥12 s election-timeout floor; a hypothetical 40 ms in-region p99 gives ≥160 ms, which is the configuration the engine is actually for. Cost is worked the same way — 100 entries/s is ~$1,300/month per node in S3 PUT charges alone, so high append rates are **financially** prohibitive, not merely slow.
  - **The honest answer to "is an object store an appropriate Raft backend at all"** is stated before anything is built on it: yes for low-write-rate, durability-dominated clusters where the election timeout can be seconds and instance loss must not lose the vote, and as the off-node tier that makes backup/restore possible at all; **no** as a general replacement for a local WAL. A spec presenting this as free would be worse than useless.
  - **Per-provider consistency, durability and conditional-write tables, with the confirmed-vs-OPEN column load-bearing.** S3 and GCS document strong read-after-write *and* list-after-write explicitly; **list-after-write is OPEN for Azure Blob, OCI and OSS** and is a spike item closed empirically as well as documentarily, because a lagging listing yields a silently short log — the one consistency failure the engine cannot detect at runtime. Azure's durability-on-response turns out to be **account configuration the engine does not control**: ZRS is the only redundancy mode documented as synchronously writing all replicas before success, LRS is single-datacenter, and GRS's cross-region copy is asynchronous. OCI's "2xx ⇒ durable" wording is OPEN — recorded as OPEN in OCI's own row rather than covered by the four confirmed providers' wording.
  - **Fencing decided**: compare-and-swap on `term`/`voted_for`, at **zero extra round trips**, resting on the observation that a second writer cannot cause a Raft safety violation without first passing through one of those two objects — **plus create-only preconditions on log appends**, because that chokepoint argument covers safety and not corruption: a stale leader appends without ever changing its term, so nothing makes it touch either object before interleaving log entries with the rightful owner's, and the damage surfaces only at the next recovery. The precondition rides on a PUT already in flight, so it costs nothing. Snapshot writes and DELETEs stay unconditional as a **stated residual rather than a covered case** — claiming a total fence an operator might rely on would be the more dangerous error. It detects and latches terminally; it does not coordinate. Azure Blob leases — the strongest primitive on offer — were considered and rejected (a finite lease needs a renewal timer in an engine with no async path; an infinite one turns a crash into a manual lease break). Where a provider cannot express the precondition it is a **compile error, never a silent degradation** — and **Alibaba OSS may be exactly that provider**: only `x-oss-forbid-overwrite` is confirmed, ETag-based overwrite CAS is not, and the vendor documents the header as *silently ignored when bucket versioning is enabled*. A fence believed in and absent is worse than no fence, so the real tier carries a live negative control rather than asserting the header was sent.
  - **Backup and restore are in scope** — `object_store_backup` plus a `cmd/raft_object_backup` CLI, off the Raft path structurally (it never instantiates the engine) — with the manifest written **last** so its presence is the commit point, and `verify` checking index contiguity, checksums and term ordering so a smeared backup is *detectable* rather than warned about. Two deliberately separate restore verbs: **clone** (same identity, owner epoch bumped so a returning original fences itself out) and **seed** (new cluster from a snapshot, membership replaced, term reset, vote cleared, log emptied). Merging them into one `restore` with flags is how an operator produces a split-brain by typing one word. Snapshot retention is added but documented as **not** a backup: retained copies share a bucket, prefix, credential and blast radius with the thing they would recover from.
  - **SDK asymmetry followed rather than flattened**: S3 needs **zero build change** (`aws-sdk-cpp`'s `s3` feature is already listed and already found at `CMakeLists.txt:1072`); GCS extends the existing opt-in `gcp` vcpkg feature with `storage`; OCI hand-rolls over the existing `oci_signing`, needing a new raw-bytes path on `oci_http_client` (whose `request()` parses every response as JSON — a control-plane assumption); Azure Blob is hand-rolled over httplib with the AAD bearer token `azure_client_config` already produces, with `azure-storage-blobs-cpp` recorded as the refutable Task 0 fallback rather than a new unconditional dependency. Test tiers follow the same asymmetry honestly: **LocalStack for S3 and Azurite for Azure Blob** are real advantages those two have; GCS gets `fake-gcs-server`-or-mock pending a fidelity check on generation preconditions specifically; OCI and Alibaba get signature-verifying mocks because no credible emulator exists.
- **Alibaba Cloud provider implemented end to end** (`.kiro/specs/alibaba-cloud-services/`, tasks 1-9): `alibaba_ess_quorum_manager` and `alibaba_oss_persistence_engine`, on a hand-rolled signing/transport foundation with no vendor SDK — plus an account, OIDC role, multi-zone scaling group and bucket provisioned live. Seven test suites green; two tiers (unit + signature-verifying mock) plus an opt-in real tier wired into `real-cloud-tests.yml` using Alibaba's **official** OIDC action rather than the hand-rolled exchange the OCI job carries (OCI hand-rolls only because its option was third-party — recorded in the amended Requirement 17.2).
  - **The live tier earned its place immediately.** The first real `PutObject` failed `SignatureDoesNotMatch`: OSS V4 signs `Content-Type` when present, httplib sets that header from its body-overload argument, and the client wasn't signing it. This is the **exact inverse** of `oci_http_client`'s rule (OCI doesn't sign content-type, so there you *withhold* it) — the OCI rule was copied across and produced a client wrong in one line. Diagnosis took one step because OSS echoes the canonical request it computed; the repo's "the service's own error output is the specification" lesson holds again.
  - **The mock server now makes that class of bug locally catchable** (Requirement 16.6): it verifies both signing schemes from the bytes that actually arrived, never by calling the client's own signing code. Proven live by a negative control — stubbing the authorizers to return true failed all 10 signature cases — and it reproduces the content-type regression as a standing test.
  - **Two further defects found by building and probing rather than reading**: the control-plane client was putting **two `Host` headers** on every request (HTTP/1.1 says MUST NOT; httplib's server tolerates it, a stricter front end need not), and cpp-httplib **rewrites the query string after signing** — settled live rather than left as a documented risk, since Alibaba canonicalises from parsed parameters and accepts it (spike Finding 8; the one untested shape, a value containing a space, is recorded).
  - **Provisioning corrections, each learned from a failing call**: OIDC identity-provider operations live under `ims`, not `ram` (the *server* rejects it on ram/2015-05-01, so it is a product split, not stale CLI metadata); `Fingerprints` is mandatory unlike AWS's auto-derived thumbprint, and is computed from the issuer's live TLS chain rather than hardcoded; custom policies cap at 5 versions, so the provisioning script prunes before creating one.
  - **Honest gap**: the ESS quorum manager is built, unit- and mock-tested, but **has never run against live ESS/ECS** — its response shapes, pagination limits and the `RemoveInstances` decrement parameter are documentation-derived. Its three cheap real cases cost nothing and are the next check. The certificate provider the TODO entry named was **descoped on cost grounds** (no Alibaba CA purchased), recorded as revivable.

### What Changed (August 13, 2026)

- **Alibaba Cloud provider spec authored** (`.kiro/specs/alibaba-cloud-services/` — requirements/design/tasks; no implementation yet). Scope per doc/TODO.md's entry plus its persistence-section mandate — **minus the certificate provider, descoped the same day by operator decision on cost grounds** (no Alibaba CA, private or public, will be purchased; the spec's Requirement 12 records the rationale and the conditions under which it could be revived, and the drafted requirements stay recoverable from this file's git history): `alibaba_ess_quorum_manager` (ESS scaling-group capacity + ECS tags, `aws_asg_quorum_manager` semantics — no heartbeat tier, no stockout ladder until live failures justify them), `alibaba_oss_persistence_engine` (one object per state item, in-memory mirror, synchronous durable PUTs as the fsync equivalence — stricter-than-file-engine corruption handling, single-slot snapshot parity, retention/fencing deferred to the cross-provider persistence spec). Load-bearing decision recorded as refutable in the spike: **no vendor SDK** — hand-rolled ACS3-HMAC-SHA256 (control plane) and OSS V4 (data plane) signing on the OCI provider's proven shape, since the general C++ SDK is not in vcpkg and the OSS-only one would add a second HTTP stack and bury durability-critical retry semantics. Test pyramid mirrors OCI (golden-vector unit tier, signature-verifying mock server, exit-77 real tier never registered with CTest); CI wiring specs AssumeRoleWithOIDC federation and ships fail-closed — **no Alibaba account exists**, and every task needing a live run is marked blocked-on-account rather than checkable against the mock. (AWS CloudWatch, Azure Monitor, GCP Cloud Monitoring, OCI Monitoring, Alibaba CloudMonitor) delivered as the config-only integrations the TODO prescribed** — example configs under `docker/cloud-monitoring/`, operator docs in `doc/cloud_vendor_monitoring.md`, and both testing tiers wired. No new `kythira::metrics` code: the node's existing OTLP (or, for OCI, Prometheus) emission is routed by an OpenTelemetry Collector or the vendor's own agent.
  ...
  - **Real-cloud tier: five lightweight `<provider>-monitoring` jobs** in real-cloud-tests.yml (no C++ build — a host Collector binary, one synthetic OTLP probe, the vendor's own query API as the oracle; `scripts/real-cloud-monitoring/`), each disabled by default behind `REAL_CLOUD_TESTS_<PROVIDER>_MONITORING_ENABLED` + the master switch, each failing closed by naming its missing variables. AWS adds a `cloudwatch-monitoring` IAM bundle scoped to the `/kythira/chaos-node/*` log groups. Honest status recorded in every doc that touches them: Azure/GCP are runnable once their documented monitoring-specific values are set; **OCI (Management Agent install key/installer URL) and Alibaba (no account exists) are wired but have never run against the live services**. **AWS was verified against the real service the same day** (the `cloudwatch-monitoring` bundle added to the CI role via `provision-oidc-role.sh` with the full five-bundle list — before/after `get-role-policy` diff confirmed pure addition — then dispatch run 31711151464: probe metric extracted into `Kythira/ChaosNode` by real CloudWatch 9 s after ingestion, log record confirmed at 10 s). One dispatch-mechanics finding from that run: on `workflow_dispatch`, GitHub materializes every unprovided boolean input as `false`, so the `inputs-null → repository variable` fallback the jobs' `if:` expressions carry only ever applies to scheduled runs — a manual dispatch must pass `run_real_cloud_tests=true` explicitly (the first dispatch skipped every job silently). — three `kythira::diagnostic_logger` implementations, paired with their ecosystems and verified against real agents** (the same-day follow-up the TODO item prescribed; before this only OTLP had a logging half).
