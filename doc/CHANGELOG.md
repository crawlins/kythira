...
- **Logging alongside the self-hosted metrics backends — three `kythira::diagnostic_logger` implementations, paired with their ecosystems and verified against real agents** (the same-day follow-up the TODO item prescribed; before this only OTLP had a logging half).
