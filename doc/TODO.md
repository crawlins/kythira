...
| `oci-build-cache` | 1/12 | Requirements, design and tasks written September 4, 2026. **Started September 5, 2026**: task 4 (the CMake launcher) is complete and locally verified, and tasks 2, 3 and 5 have their artifacts written and exercised without a tenancy -- the provisioning and audit scripts, the composite action, and the documentation. Nothing is wired into a workflow yet and nothing is provisioned; tasks 1 and 2's remaining halves need the OCI tenancy and a CI run, and everything after them is gated on those. One premise has also changed since this row was written: `ci.yml` still asks for vcpkg's `x-gha` binary cache and vcpkg has **removed** that backend, so the layer this spec replaces is gone rather than merely evicted, and a cache-key miss now costs a full dependency rebuild in every job -- 70 m 30 s in run 33997439418's `Build & Test (g++-13, x64)`. Moves the vcpkg binary cache (`x-aws`) and this project's compiler cache (sccache's `s3` backend, replacing ccache in CI only) into one OCI Object Storage bucket through the S3 Compatibility API. Motivated by `scripts/prune-actions-caches.sh`'s own measurement -- the repository sits at GitHub's 10 GB cache ceiling (10.37 GB across 41 entries, 4.49 GB of them vcpkg archives that are never pruned) and evicts LRU -- and by what that does to every run: on September 4 the four `build-and-test` legs built in 17-26 minutes against the ccache spec's 11 m 59 s warm figure, with "Restore ccache" taking 1-3 s, i.e. cold every time; the stdexec leg's memory-pressure failures are, by its own comment, the cold-cache case. Pre-registered cost about $0.50/month (`doc/sccache_dogfood_cost_estimate.md`), the 1-5 TB/month of runner traffic being inside OCI's 10 TB free egress. Writers are pushes to `main` only, enforced by IAM (a read-only user for `pull_request`, nothing for fork PRs, and the build proceeds cache-less in every failure); measurement on a throwaway PR (task 1) precedes any permanent wiring, because sccache has no direct mode and its PCH handling differs from ccache's, so the per-leg cacheable fraction is the number that decides whether a leg moves. Explicitly not the Redis-gateway dogfood, which the same cost document estimates at about $55/month and which this spec is the baseline for. |
