...
- **`multi_raft_scale_test` and `multi_raft_driver_agreement_test` sit
  outside the `heavy_tu` job pool, and a cold build dies on them (found
  September 9, 2026 by `.kiro/specs/oci-build-cache/`).** The
  `Full suite (stdexec)` leg was killed five times at objects 226, 236, 226,
  226 (`-j3`) and 227 (`-j2`) of 1115 — a death point that holds steady
  across a changed parallelism setting is one translation unit, not
  aggregate memory pressure, which is what the `-j2` experiment
  ([PR #323](https://github.com/crawlins/kythira/pull/323), closed unmerged)
  was run to distinguish and did. Those two TUs are the cluster, and
  `ci.yml`'s `heavy_tu` pool exists for exactly this failure mode; they are
  simply not in it. **The build cache hides this rather than fixing it**:
  every leg now compiles from cached objects, so the landmine only re-arms
  on a genuinely cold build — a compiler bump, a new `-D`, anything that
  moves the hash — and it will be hit at the least convenient moment, by
  someone who has no reason to connect a fresh OOM to a caching change made
  months earlier. The fix is adding both TUs to the pool as its own change
  with its own measurement, deliberately not folded into the caching work.
