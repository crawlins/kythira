...
- `.kiro/specs/` now holds **58** per-feature spec directories (re-counted
  ...
  `alibaba-cloud-services` has its **vendor-fact spike** open but is **not at
  wave 1** — that half was wrong and is corrected below; see "Pending
  Specifications". (This said *three* when it was written on
...
reason — while the work was happening rather than long after. Nine of its
twelve tasks are done and both caches are live on `main`; what is left is
one second-run threshold reading, a month-one bill that cannot be read
until there is a month, and this close-out. Worth recording here because
the spec's own premise did not survive: it argued for moving the compiler
cache out of GitHub's 10 GB Actions cache to relieve the ceiling, and
measurement showed ccache was 0.60 GiB of it. The ceiling was never the
problem. The win is real but different — eviction stopped being expensive.
...
| Spec | Tasks | Notes |
|------|-------|-------|
| `oci-build-cache` | 11/12 | **Both build caches are live on `main` and verified** (run [34296062792](https://github.com/crawlins/kythira/actions/runs/34296062792), 15/15 green, September 9, 2026). vcpkg archives via `x-aws` and this project's compiles via sccache now live in one OCI bucket reached through the S3 Compatibility API. Measured, with an unmodified control leg to anchor it: dependency installs **69–163 min → 4–7 min** with zero ports built from source; `gcp-sdk-build` **163.4 → 6.5 min**; the `Full suite (stdexec)` Build **170.3 → 4.1 min** at a **100% hit rate over 561 translation units**; every Requirement 6 threshold met by an order of magnitude. Requirement 1.3's two open questions are answered **yes** — across 3,856 compiles spanning Release, ThreadSanitizer and coverage builds there is not one non-cacheable call, so neither `kythira_test_pch` nor `-fprofile-instr-generate` defeats the cache, and Requirement 5.1's escape hatch for legs under 50% cacheable has no candidates. **Two premises in the spec did not survive contact and are corrected in it.** First, `x-gha` — the backend every workflow was asking for — had already been removed by vcpkg, so those jobs had no binary cache at all rather than a degraded one. Second, and more important for how this is described: moving the compiler cache out **did not relieve the 10 GB ceiling**. ccache was 0.60 GiB in total, not the 'eight families at 2 GB each' assumed, and 94% of the ceiling is the `vcpkg_installed/` tree caches that Requirement 3.3 deliberately keeps. What changed is that **eviction stopped mattering** — a tree-cache miss cost 69–163 minutes of rebuilding and now costs a 4–7 minute download. That is a better outcome than the one argued for, and a different one. **Every Requirement 6 threshold is met on the second consecutive `main` push** (run [34353211972](https://github.com/crawlins/kythira/actions/runs/34353211972), 15/15 green): `build-and-test` Builds of **2.23–4.05 min** against a 15-minute ceiling and 17–26 cold, the stdexec leg at **4.01 against 45** and 170.3 cold, hit rates of **99.82–100%**, and **zero non-cacheable calls in 4,494 requests**. Nothing needed investigating and no threshold was moved. **The negative-path verification found the one thing that was genuinely broken**, which is the argument for doing it: a job pointed at a wrong endpoint built *red* rather than degrading to no compiler cache, violating Requirement 3.5. sccache validates its storage backend lazily — on the first request — so neither subcommand the guard used could distinguish a healthy server that was already listening from a dead one, and it chose sccache over a dead one; every compile then exited 2 and ninja stopped one second in. Fixed in [PR #326](https://github.com/crawlins/kythira/pull/326) by deciding with a probe compile, in one script replacing a guard that had been duplicated verbatim at twelve sites. Storage measured at ~9.2 GB against the $0.50/month pre-registration (of which 2.04 GB is a throwaway measurement prefix that ages out; nobody holds `OBJECT_DELETE`, by design). **The negative-path verification took two attempts and found a second violation the first fix could not have caught**: `RUSTC_WRAPPER` was exported before the vcpkg install, where the lakers port's cargo build uses it and where no per-job guard can reach, so a dead backend would have failed rustc and taken the install with it ([PR #328](https://github.com/crawlins/kythira/pull/328)). Neither violation had ever fired, because the `vcpkg_installed/` tree cache had been hitting on every run and a hit skips the install entirely. The re-run is **15/15 green** (run [34358385332](https://github.com/crawlins/kythira/actions/runs/34358385332)) with 117 ports built from source, and it is also the first *controlled* measurement of what the vcpkg cache is worth — ten legs installing in **4.41–6.51 min** beside one at **95.01**, same commit, same L1 miss, the only variable being whether the cache was reachable. Every earlier figure for that ratio compared different runs on different days. A third defect turned up in `audit.sh`, which reported both credentials as "does not exist" while CI was authenticating with them: `try()` folded stderr into the captured value, so a diagnostic became an OCID ([PR #329](https://github.com/crawlins/kythira/pull/329)). All three are the same shape — something that could not answer being read as an answer. **A fourth of that shape turned up on September 10, 2026, and it is the one that had held task 11 open**: `audit.sh` had been reporting "no Object Storage line yet this month" about a line that was there the whole time, because it asked the Usage API for an *ungrouped* total and then filtered it by `service` — a field that API populates only when asked to group. Same window, same credentials: ungrouped returns one row with `service` null, `--group-by service` returns six of which Object Storage is one. So **the month-to-date bill is readable and is $0.051963**, against the **$0.50/month** pre-registered — and the shape of it was not anticipated anywhere: **requests are 100% of the cost** (202,831 of them), while storage (0.902 GB-months) and egress (130.11 GB) both round to **zero** under the 10 GB always-free and 10 TB egress allowances. This bill scales with round trips, not bytes, so the 30-day lifecycle rule that bounds storage does not touch the only line that costs anything. Four checks were added to `test-audit.sh` and all four fail against the previous `audit.sh`; the stub had to be taught to model `--group-by` first, because it answered with a service-labelled row either way and so could not reproduce the bug — the same "a stub more permissive than the service it stands in for tests nothing" lesson task 9 already recorded. The rule now written at the site: **when a query comes back empty, say whether the QUERY failed or the SUBJECT is absent, and never let those print the same sentence.** Remaining: one *full month* of cadence for the egress figure, on or after October 9 — 130 GB over eleven days that began before the caches were live on `main` and included a six-PR day neither confirms nor refutes the 1–5 TB band. The cost cross-reference in `doc/sccache_dogfood_cost_estimate.md` is **done**, and it reports that document's central thesis — egress is the expensive part, and OCI's allowance zeroes it — as holding, while noting that the document prices a three-node `redis_gateway_node` deployment that was never built. **Two findings outlive the spec**: a leg that only ever passes on a `pull_request` can never warm its own cache, because PR runs hold the read-only key — a deadlock that caught the stdexec leg; and `multi_raft_scale_test`/`multi_raft_driver_agreement_test` will kill a 16 GiB runner on any genuinely cold build, sitting outside the `heavy_tu` pool built for exactly that, which the cache now hides rather than fixes. **That second finding has since been measured and fixed, and the two targets it names are not the culprits**: ninja's `[N/M]` on a non-smart-terminal is a *completion* index, so the two targets it named were the last to finish before the machine stopped rather than the ones that stopped it. The killers are `multi_raft_http_benchmark_test` and `multi_raft_performance_report`, which never completed at all in either killed job and which peak at **21,114 and 16,640 MiB** under stdexec — 36.9 GiB together, against a runner's 16 GiB plus the 24 GiB swapfile that leg adds. All four are now pooled — see "Known Follow-ups". |
| `alibaba-cloud-services` | 3/12 by its own checklist, **but that checklist is stale** | **Added to this table September 12, 2026 — it had a row in neither table**, while `doc/TODO.md` said two contradictory things about it: the Current Status bullet called it "wave 1 with its vendor-fact spike open", and the Cloud Provider Support list marked it `[x]` complete. The `[x]` is the closer of the two. **What the tree actually holds**, checked rather than inferred: `alibaba_client_config`, `alibaba_http_client`, `alibaba_signing`, `alibaba_oss_client`, **`alibaba_ess_quorum_manager` at 1,049 lines** and `alibaba_oss_persistence` — the last two landed as `5fd8f14`, which is on `main` and is exactly tasks 3 and 5, both of which the spec still shows unchecked. Also present and wired: `alibaba_mock_server.hpp`, unit/mock/real test tiers for signing, the HTTP client, the OSS client, persistence and the quorum manager, `docker/alibaba_quorum_manager/`, `scripts/ci-cloud-credentials/alibaba/`, and 70 references in `real-cloud-tests.yml`. **Seven alibaba tests are registered CTest entries in a configured tree**, which is the evidence that these are built rather than merely present — file existence on its own is not, and this file has been wrong that way before. So tasks 6 through 9 look done too. **What is genuinely outstanding is small and mostly not ours**: spike sub-items **0.2, 0.3 and 0.5** (the V4/OSS canonical form, `ModifyScalingGroup` semantics, and the `AssumeRoleWithOIDC` request shape — 0.1 and 0.4 are CONFIRMED in `spike-notes.md`), task **10**, which is marked `[operator]` and needs a real Alibaba Cloud account provisioned by a human, and task **11**, live verification, which task 10 blocks. **The task count above is deliberately the spec's own, not a corrected one.** Ticking six boxes on the strength of "the files exist and build" is the move this repo's Known Follow-ups exist to warn about — a per-task pass against each acceptance criterion is owed, by someone who can check them, and until that happens the honest thing is to record that the checklist and the tree disagree rather than to pick a number. |
...
- **SUPERSEDED (September 12, 2026): the split this entry measured no longer
  exists, and the conclusion drawn from it was refuted in between.** There are
  now **zero** `key: ccache-` entries in `ci.yml` — sccache replaced ccache on
  every moved leg and its objects live in the OCI bucket, not in the Actions
  cache (`.kiro/specs/oci-build-cache/`). The 5.61 GB figure below therefore
  describes nothing. **And the premise was already wrong before that**: task 8
  of that spec re-measured the same quota and found ccache was **0.60 GiB in
  total**, not 5.61, with **94% of the ceiling being the `vcpkg_installed/`
  tree caches** that Requirement 3.3 deliberately keeps — so moving the
  compiler cache out could never have relieved the ceiling, and did not. What
  actually changed is that **eviction stopped being expensive**: a tree-cache
  ...
  **The original measurement, August 6, 2026**, kept because the accounting
  ...
  against — `actions/cache/usage` reported **10.44 GB across 34 caches**,
  against a documented 10 GB per-repo ceiling, so GitHub was already evicting
  entries LRU. Split: **ccache 5.61 GB** (31 entries) and **vcpkg 4.83 GB**
  (3 entries). vcpkg permanently occupies ~4.8 GB, leaving
...
- **RESOLVED, and confirmed by re-running its own checks (September 12,
  ...
  way they were originally established: `ci.yml` contains **22** occurrences
  of `KYTHIRA_KCONFIG`, **9** of `KYTHIRA_KCONFIG_STRICT` and **6** of
  `ci_full_defconfig` — the first legs configure at lines 295-296 with
  `-DKYTHIRA_KCONFIG=configs/ci_full_defconfig -DKYTHIRA_KCONFIG_STRICT=ON`.
  **Requirement 5.1 is in force**, `kythira_kconfig_require()` can fire, and
  the defconfig is named for a consumer that now uses it. The entry is kept
  because the diagnosis is worth reading and because *an entry that outlives
  its problem is itself the failure mode this section documents* — a status
  line nobody re-ran.

  **The original finding, August 10, 2026** — Kconfig strict mode is not
  exercised anywhere, and `ci_full_defconfig` is dead config, found while
  making strict mode runnable locally. Three facts, each checked rather than
  inferred:
...
- **FIXED September 9, 2026: the four multi-Raft TUs that killed the
  `Full suite (stdexec)` leg are in the `heavy_tu` job pool** (found by
  `.kiro/specs/oci-build-cache/`; measured and fixed the same day). **The
  diagnosis this entry previously carried was wrong, and how it was wrong is
  the part worth keeping.** It named `multi_raft_scale_test` and
  `multi_raft_driver_agreement_test`, because those are the targets ninja
  named at the objects the leg died at — 226, 236, 226, 226 (`-j3`) and 227
  (`-j2`) of 1115 — and a death point that holds steady across a changed
  parallelism setting looks like one translation unit rather than aggregate
  pressure. **Ninja's `[N/M]` counter on a non-smart-terminal is a completion
  index, not a start index**: it buffers an edge's output and prints the
  status line when that edge *finishes*. Those two were the last TUs to
  finish before the machine stopped making progress, not the ones that
  stopped it — and the stability across `-j2` and `-j3` is precisely what a
  completion index does, since what finishes last is fixed by declaration
  order rather than by parallelism.
  **What the logs say once read that way**: in job `102095938462` (run
  [34236640491](https://github.com/crawlins/kythira/actions/runs/34236640491))
  `[226] multi_raft_scale_test` completed at 14:56:54 and the runner took a
  shutdown signal at 16:03:21 — 66 minutes later, with not one further edge
  completing. Differencing every `tests/` object in a configured tree against
  the ones that job actually completed leaves **exactly two**:
  `multi_raft_http_benchmark_test.cpp.o` and
  `multi_raft_performance_report.cpp.o`. Job `102104469816` (run
  [34239130419](https://github.com/crawlins/kythira/actions/runs/34239130419))
  has the same signature — 523 edges done, the same two missing, then 94
  minutes of nothing — and on the last green cold run (job `101157909873`)
  those two did finish, 61 and 70 minutes after
  `multi_raft_driver_agreement_test`.
  **They are the three callers of `kythira_wire_multi_raft_bench()`**, which
  links Beast, Proxygen, cpp-httplib, Folly and protobuf into a translation
  unit that already instantiates the whole multi-Raft stack — the shape of
  `three_way_http_transport_equivalence_test`, and far worse: under stdexec
  two of them peak at **21,114 and 16,640 MiB** against 5,967 MiB for the
  pool's previous worst case. That pair is the whole failure in one line of
  arithmetic — 36.9 GiB against a runner's 16 GiB plus the 24 GiB swapfile
  this leg's Build step adds, and they are declared consecutively so `-j3`
  starts them together. Pool membership is now set inside that function, so a fourth
  caller is covered without anyone remembering; `multi_raft_scale_test`
  (4,044 MiB under stdexec, 1,593 under folly) is pooled at its own
  declaration. **`time -v`'s `ru_maxrss` could not have found this** — on a
  7.6 GiB box the three TUs above its resident ceiling report 6,599, 6,757 and
  6,594 MiB through it: three numbers within 3% of each other, and of an
  ordinary heavy TU, describing translation units that differ by a factor of
  three. The figures above are peak `max(VmRSS + VmSwap)` sampled from
  `/proc`, which for a TU that is *not* being evicted agrees with `ru_maxrss`
...
- **FIXED September 10, 2026: the two translation units that cost 21,114 and
  ...
  **Two translation units in `tests/` cost 21,114 and 16,640 MiB, and over an
  ...
  these targets went through within 90 seconds of each other. That is the
  counter-evidence this repo already had on file ("a fully cold build is
  survivable"), and the explanation is simply that the leg is folly.
