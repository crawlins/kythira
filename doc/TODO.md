...
| `grpc-transport` | 12.5/13 phases | Tasks 1–12 are implemented and **CI-verified**: `proto/raft.proto`, the `raft_grpc_transport` target, `include/raft/grpc_transport{,_impl}.hpp`, `grpc_exceptions.hpp`, `grpc_message_conversion.hpp`, plus `grpc_transport_conversion_property_test`, `grpc_transport_integration_test` and `grpc_transport_example_test` — all three of which pass on all four `Build & Test` legs. Its `tasks.md` had flagged Tasks 1.5/13 as needing "a build machine with gRPC/Protobuf present"; CI *is* one, because `grpc` is an unconditional `vcpkg.json` dependency (which is why nothing in `ci.yml` mentions gRPC by name, and why this went unnoticed). What genuinely remains of Task 13 is narrower still, as of August 10, 2026: **both configure checks are done, leaving only the performance sanity pass.** Graceful degradation verified by configuring with `-DCMAKE_DISABLE_FIND_PACKAGE_gRPC=ON`: configure succeeds, the transport, its tests and its example each skip with a named message, and — checked rather than assumed from the message — the target list drops from 518 to 505 with 0 of the 10 `grpc`-named targets present. Strict mode verified with a matched pair: `-DKYTHIRA_KCONFIG=configs/ci_full_defconfig -DKYTHIRA_KCONFIG_STRICT=ON` plus that same disable fails with `CONFIG_GRPC_TRANSPORT=y (strict mode) but grpc was not found`, and the identical configure *without* the disable gets past that check entirely — so the check fires on the condition it names and not on the configuration. The control's own failure is informative and not a gRPC problem: it stopped later on `CONFIG_POCO_DISCOVERY=y`, because the local vcpkg tree had no Poco DNSSD. **Correction, same day:** that entry first read "CI's image has Poco DNSSD and ion, this one does not". That was assumed, and it is false in both halves — see the Kconfig entry under "Known Follow-ups" for what `ci.yml` actually does. Poco DNSSD and ion-c have since been installed locally; the remaining strict-mode blocker on this machine is `CONFIG_GCP_SDK=y`, which needs `--x-feature=gcp` (google-cloud-cpp with the full Compute API surface — deliberately out of the default dependency set) |
