...
- `.kiro/specs/` now holds **57** per-feature spec directories (re-counted
  September 3, 2026 with `ls -d .kiro/specs/*/`; this line said 45 for a
  month while twelve were added). Three have zero implementation commits —
  `redis-compatible-kv`, `coap-transport-multi-raft`, `elastic-shard-capacity`
  — and `alibaba-cloud-services` is at wave 1 with its vendor-fact spike open;
  see "Pending Specifications" below
...
has since grown to 57 per-feature spec directories, most now complete (see
