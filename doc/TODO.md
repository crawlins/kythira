...
| `multi-machine-placement` | 0/10 | Requirements, design and tasks written August 31, 2026; zero implementation commits. **The one thing Tier E exists to introduce — a real network between the nodes — is still absent from every row this project has produced.** `.kiro/specs/multi-raft-host-binary/` delivered Tier E rows on three rootless-Podman containers, and the inter-node probe measured **0.7–0.9 ms** between them, which is the number loopback gives because it *is* loopback: containers on one host share a kernel, a scheduler and a network stack. Every inter-round-interval figure this project has published is a loopback figure — 0.75–0.91 ms on a cloud instance, 1.89–2.26 ms on the development machine — which means task 11's response-driven-pacing hypothesis has never been testable, only fittable: on loopback the round trip barely varies. A real network moves it one to two orders of magnitude, and **that is the design's actual purpose**; the throughput number is a by-product. Shape 2 is N+1 instances (N hosts, driver on its own, for the CPU accounting Tier C already bought), placement as a swept axis (one AZ, several AZs, cluster placement group), the RTT/bandwidth matrix between every ordered pair before the window, and discovery that does not need a hand-edited file — recommending a tag scan over the `peer_discovery` concept the tree already has, while **keeping the static list as the default for a measured row**, because discovery inside a window is a cost in that window. Estimated at roughly **$0.70 per placement, ~$2 for all three** — an order of magnitude over Shape 1's $0.09 and still small; the number to watch is the orphan, four instances left running being $32/day, which is why every instance gets its own dead-man switch rather than trusting the controlling process. Writing it corrected one error worth recording: the spec initially claimed placement groups were a resource class the leak audit could not see, and **`audit-aws-leaks.sh` already checks them**, with a comment saying Tier E's shape uses one — Shape 1's author anticipated this shape and left the check unfired. Requirement 4.5 makes the container failure impossible to repeat silently: an RTT within an order of magnitude of loopback means a container-shaped row, and the row must say so. **Does not deliver Tier D**, which needs a volume and a run and nothing else |
