 ...
 *
 * Property: For any type that satisfies the network_client concept, all RPC methods should return
 * kythira::Future types
 */
...
BOOST_AUTO_TEST_CASE(test_network_server_concept_compliance, *boost::unit_test::timeout(30)) {
    ...
    public:
        ...
        auto register_append_entries_handler(std::function<kythira::append_entries_response<>(
                                                 const kythira::append_entries_request<>&)>
                                                 handler) -> void {}
...
BOOST_AUTO_TEST_CASE(test_concept_constraints_with_invalid_types, *boost::unit_test::timeout(30)) {
    ...
    public:
        auto send_request_vote(std::uint64_t target, const kythira::request_vote_request<>& request,
                               std::chrono::milliseconds timeout) -> invalid_future {
