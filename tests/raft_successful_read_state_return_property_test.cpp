 ...
 *
 * Property: For any successful heartbeat collection during read, the current state machine state is
 * returned.
 * **Validates: Requirements 7.2**
 */
BOOST_AUTO_TEST_CASE(raft_successful_read_state_return_property_test,
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        BOOST_TEST_MESSAGE("Simulated " << successful_responses << " successful responses out of "
                                        << follower_count << " followers (required: "
                                        << required_successful_followers << ")");
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(heartbeat_futures), test_timeout);

        try {
            auto results = std::move(collection_future).get();

            // Property: Successful heartbeat collection should enable state return
            BOOST_TEST_MESSAGE("✓ Successful heartbeat collection returned " << results.size()
                                                                             << " responses");
            ...
            if (higher_term_in_results == 0) {
                ...
                if (total_success_count >= majority_count) {
                    ...
                    std::vector<std::byte> returned_state =
                        expected_state;  // In real impl, this comes from state machine
                    ...
                    BOOST_TEST_MESSAGE(
                        "✓ State machine state correctly returned after successful heartbeat "
                        "collection");
                } else {
                    BOOST_TEST_MESSAGE("Insufficient majority ("
                                       << total_success_count << "/" << majority_count
            ...
            BOOST_TEST_MESSAGE(
                "Heartbeat collection failed, state correctly not returned: " << e.what());
        }
    }
    ...
    // Test with guaranteed successful majority
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(success_futures), test_timeout);
    ...
    // Test with mixed success/failure but sufficient majority
    {
        ...
        // Create 2 successful and 2 failed responses (with leader = 3 successful total, majority of
        // 5 is 3)
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(mixed_futures), test_timeout);
        ...
        BOOST_TEST_MESSAGE(
            "✓ Mixed responses with sufficient majority correctly enables state return");
    }
    ...
    // Test empty state return
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(single_future), test_timeout);
        ...
        BOOST_TEST_MESSAGE(
            "✓ Empty state correctly returned after successful heartbeat collection");
    }
