...
BOOST_AUTO_TEST_CASE(test_coap_client_namespace, *boost::unit_test::timeout(30)) {
    ...
    using TestTypes =
        kythira::coap_transport_types<kythira::json_rpc_serializer<std::vector<std::byte>>,
                                      kythira::noop_metrics, kythira::console_logger>;
...
BOOST_AUTO_TEST_CASE(test_namespace_migration_completeness, *boost::unit_test::timeout(30)) {
    ...
    using TestTypes =
        kythira::coap_transport_types<kythira::json_rpc_serializer<std::vector<std::byte>>,
                                      kythira::noop_metrics, kythira::console_logger>;
    using FutureType =
        typename TestTypes::template future_template<kythira::request_vote_response<>>;
    using HttpClientType = kythira::cpp_httplib_client<TestTypes>;
    using CoapClientType = kythira::coap_client<TestTypes>;
    using ConnectionType = kythira::Connection<std::uint64_t, unsigned short, FutureType>;
    using ListenerType = kythira::Listener<std::uint64_t, unsigned short, FutureType>;
    using SimClientType = kythira::simulator_network_client<
        FutureType, kythira::json_rpc_serializer<std::vector<std::byte>>, std::vector<std::byte>>;
    using SimServerType = kythira::simulator_network_server<
        FutureType, kythira::json_rpc_serializer<std::vector<std::byte>>, std::vector<std::byte>>;
