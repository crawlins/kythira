...
struct cluster_fixture {
    ...
    explicit cluster_fixture(std::size_t node_count) {
        ...
        for (std::size_t i = 0; i < node_count; ++i) {
            ...
            // Use the node_id directly as the simulator address (it will convert to string
            // internally)
        ...
        network_simulator::NetworkEdge edge(std::chrono::milliseconds{0},
                                            1.0);  // No latency, 100% reliability
        for (std::size_t i = 0; i < node_count; ++i) {
    ...
    auto wait_for_leader(std::chrono::milliseconds timeout)
        -> std::optional<test_raft_types::node_id_type> {
...
BOOST_AUTO_TEST_CASE(test_cluster_bootstrap, *boost::unit_test::timeout(30)) {
    ...
    std::set<test_raft_types::node_id_type> unique_ids(cluster.node_ids.begin(),
                                                       cluster.node_ids.end());
