        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(dir_path)) {
            if (entry.is_regular_file() &&
                ...
                if (file_path.find("future_usage_consistency_property_test.cpp") !=
                        std::string::npos ||
                    file_path.find("complete_conversion_validation_property_test.cpp") !=
                        std::string::npos ||
                    ...
                    file_path.find("coap_transport.hpp") !=
                        std::string::npos ||  // Contains std_coap_transport_types
                    file_path.find("http_transport.hpp") !=
                        std::string::npos ||  // Contains std_http_transport_types
                    file_path.find("coap_multicast_group_communication_property_test.cpp") !=
                        std::string::npos ||
                    file_path.find("http_transport_return_types_property_test.cpp") !=
                        std::string::npos ||
                    file_path.find("http_transport_types_property_test.cpp") != std::string::npos ||
                    file_path.find("coap_comprehensive_error_handling_property_test.cpp") !=
                        std::string::npos ||
                    file_path.find(
                        "network_simulator_concurrent_operations_integration_test.cpp") !=
                        std::string::npos ||
                    file_path.find("raft_concurrent_read_efficiency_property_test.cpp") !=
                        std::string::npos ||
                    file_path.find(
                        "network_simulator_connection_management_integration_test.cpp") !=
                        std::string::npos ||
                    ...
                    file_path.find("coap_performance_validation_example.cpp") !=
                        std::string::npos) {
                    continue;
                }
...
BOOST_AUTO_TEST_CASE(property_no_remaining_folly_future_in_public_interfaces,
    ...
    if (std::filesystem::exists(include_dir)) {
        for (const auto& entry : std::filesystem::recursive_directory_iterator(include_dir)) {
            if (entry.is_regular_file() &&
                ...
                // folly::Future Also skip transport headers that define alternative transport types
                // (folly_*_transport_types) Also skip test helper files that are used for testing
                // purposes
                if (file_path == kythira_future_impl_path || file_path == legacy_future_impl_path ||
                    file_path ==
                        "include/raft/coap_transport.hpp" ||  // Contains folly_coap_transport_types
                    file_path ==
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(examples_dir)) {
            if (entry.is_regular_file() &&
                ...
                if (file_path.find("migration_guide_example.cpp") != std::string::npos ||
                    file_path.find("coap_transport_basic_example_fixed.cpp") !=
                        std::string::npos) {  // Demonstrates std_coap_transport_types
                    continue;
                }
                ...
                if (std::regex_search(content, folly_future_regex)) {
                    violations.push_back(file_path +
                                         ": example uses folly::Future instead of kythira::Future");
                }
            }
        }
    }
    ...
    if (!violations.empty()) {
        std::string error_msg =
            "Remaining folly::Future usage in public interfaces found (Requirements 9.2):\n";
        for (const auto& violation : violations) {
            error_msg += "  - " + violation + "\n";
        }
        BOOST_FAIL(error_msg);
    ...
    BOOST_TEST_MESSAGE(
        "No remaining folly::Future usage in public interfaces - conversion complete");
}

BOOST_AUTO_TEST_CASE(property_kythira_future_implementation_uses_folly_internally,
