...
BOOST_AUTO_TEST_CASE(test_certificate_loading_failure_nonexistent_file,
...
BOOST_AUTO_TEST_CASE(test_certificate_loading_failure_invalid_format,
                     *boost::unit_test::timeout(30)) {
    using test_types =
        kythira::http_transport_types<kythira::json_rpc_serializer<std::vector<std::byte>>,
                                      kythira::noop_metrics, folly::CPUThreadPoolExecutor>;
...
BOOST_AUTO_TEST_CASE(test_certificate_chain_validation_success, *boost::unit_test::timeout(30)) {
    ...
    try {
        ...
        BOOST_TEST_MESSAGE(
            "Certificate validation error (expected for self-signed cert): " << e.what());
        BOOST_TEST(true);
    }
...
BOOST_AUTO_TEST_CASE(test_expired_certificate_detection, *boost::unit_test::timeout(30)) {
    ...
    try {
        ...
        BOOST_TEST_MESSAGE(
            "Certificate validation error (expected for expired cert): " << e.what());
        BOOST_TEST(true);
    }
