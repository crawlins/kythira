 ...
 *
 * Property: For any read_state operation, leader status is verified by collecting heartbeat
 * responses from majority.
 * **Validates: Requirements 7.1**
 */
BOOST_AUTO_TEST_CASE(raft_read_linearizability_verification_property_test,
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        try {
            ...
            BOOST_TEST_MESSAGE("✓ Linearizability verification collected " << results.size()
                                                                           << " responses");
            ...
            // Property: If higher term responses are detected, linearizability verification should
            // fail
            if (higher_term_in_results > 0) {
                BOOST_TEST_MESSAGE("✓ Higher term detected ("
                                   << highest_term_seen
    ...
    // Test with empty futures vector (should fail)
    {
        ...
        BOOST_TEST_MESSAGE(
            "✓ Empty futures vector correctly rejected for linearizability verification");
    }
    ...
    // Test all higher term responses (leadership definitely lost)
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(higher_term_futures), test_timeout);
        ...
        BOOST_TEST_MESSAGE(
            "✓ All higher term responses correctly detected for linearizability verification");
    }
