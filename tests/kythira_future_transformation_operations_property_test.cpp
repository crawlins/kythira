...
BOOST_AUTO_TEST_CASE(property_future_thenValue_transformation, *boost::unit_test::timeout(30)) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto transformed_future =
            std::move(future).thenValue([multiplier](int value) { return value * multiplier; });
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto transformed_future = std::move(future).thenValue(
            [suffix](const std::string& value) { return value + suffix; });
...
BOOST_AUTO_TEST_CASE(property_future_thenValue_void_handling, *boost::unit_test::timeout(30)) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto transformed_future =
            std::move(void_future).thenValue([return_value]() { return return_value; });
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        bool callback_executed = false;
        auto transformed_future =
            std::move(void_future).thenValue([&callback_executed]() { callback_executed = true; });
...
BOOST_AUTO_TEST_CASE(property_future_thenError_exception_handling, *boost::unit_test::timeout(30)) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto recovered_future =
            std::move(future).thenError([recovery_value](std::exception_ptr ex) -> int {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto result_future =
            std::move(future).thenError([&error_handler_called](std::exception_ptr ex) -> int {
...
BOOST_AUTO_TEST_CASE(property_future_thenError_void_handling, *boost::unit_test::timeout(30)) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        bool error_handler_called = false;
        auto recovered_future =
            std::move(future).thenError([&error_handler_called](std::exception_ptr ex) {
                BOOST_CHECK(ex != nullptr);
                error_handler_called = true;
            });
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        bool error_handler_called = false;
        auto result_future = std::move(future).thenError(
            [&error_handler_called](std::exception_ptr ex) { error_handler_called = true; });
...
BOOST_AUTO_TEST_CASE(property_future_ensure_cleanup_execution, *boost::unit_test::timeout(30)) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto ensured_future =
            std::move(future).ensure([&cleanup_executed]() { cleanup_executed = true; });
...
BOOST_AUTO_TEST_CASE(property_future_transformation_chaining, *boost::unit_test::timeout(30)) {
    // Property: chaining transformation operations should maintain proper type deduction and error
    // propagation Validates: Requirement 6.4
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto chained_future =
            std::move(future)
                .thenValue([multiplier](int value) { return value * multiplier; })
                .thenValue([](int value) -> std::string { return std::to_string(value); })
                .thenValue([suffix](const std::string& value) { return value + suffix; });
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto chained_future = std::move(future)
                                  ...
                                  .thenValue([](int value) { return value + 100; });
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto chained_future = std::move(future)
                                  ...
                                  .ensure([&cleanup_executed]() { cleanup_executed = true; })
                                  .thenValue([](int value) { return value + 10; });
...
BOOST_AUTO_TEST_CASE(property_future_transformation_exception_safety,
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto transform_future = std::move(future).thenValue(
            [error_message](int value) -> int { throw std::runtime_error(error_message); });
...
BOOST_AUTO_TEST_CASE(property_future_transformation_move_semantics,
                     *boost::unit_test::timeout(30)) {
    ...
    for (std::size_t i = 0; i < num_iterations; ++i) {
        ...
        auto transformed_future = std::move(future).thenValue(
            [suffix](std::string&& value) { return std::move(value) + suffix; });
