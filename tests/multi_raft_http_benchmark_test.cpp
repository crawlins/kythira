...
auto report(const benchmark_result& row) -> void {
    std::ostringstream out;
    out << std::fixed << std::setprecision(1);
    out << "  " << row._transport << " / " << row._serializer << " / " << row._scenario
        << ": nodes=" << row._nodes << " groups=" << row._groups << " value=" << row._value_bytes
        << "B in_flight=" << row._in_flight << " | " << std::setprecision(1) << row._ops_per_second
        << " ops/sec"
        << " | p50=" << us(row._p50) << "us p95=" << us(row._p95) << "us";
...
auto throughput_row(std::size_t operations, std::size_t in_flight, std::size_t value_bytes,
                    double floor_ops_per_second) -> benchmark_result {
    kv_cluster<Transport> cluster{standard_cluster_options()};
    BOOST_REQUIRE_MESSAGE(cluster.await_all_leaders(k_election_budget),
                          Transport::name() << ": no leader on every shard within budget");
    ...
    warmup._operations = std::max<std::size_t>(in_flight, operations / 8);
    ...
    // test flaky in exactly the way doctrine warns against.
    BOOST_TEST_MESSAGE(
    ...
    // batching, a lost connection pool) does not.
    BOOST_CHECK_MESSAGE(row._ops_per_second >= floor_ops_per_second,
                        Transport::name()
                            << ": " << row._ops_per_second
                            << " ops/sec is below the sanity floor of " << floor_ops_per_second);
    return row;
}
...
BOOST_AUTO_TEST_CASE(write_throughput_by_transport, *boost::unit_test::timeout(1800)) {
...
BOOST_AUTO_TEST_CASE(write_throughput_by_rpc_serializer, *boost::unit_test::timeout(1800)) {
...
BOOST_AUTO_TEST_CASE(write_throughput_by_value_size, *boost::unit_test::timeout(1800)) {
