 ...
 *
 * Property: For any cancelled future collection operation, all pending futures are properly cleaned
 * up.
 * **Validates: Requirements 2.5**
 */
BOOST_AUTO_TEST_CASE(raft_collection_cancellation_cleanup_property_test,
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        // Test 1: Cancellation via timeout
        {
            ...
            auto collection_future = raft_future_collector<kythira::append_entries_response<
                std::uint64_t, std::uint64_t>>::collect_majority(std::move(long_futures),
                                                                 std::chrono::milliseconds(
                                                                     200));  // Short timeout

            try {
                auto results = std::move(collection_future).get();
                BOOST_TEST_MESSAGE("⚠ Collection unexpectedly succeeded with " << results.size()
                                                                               << " results");
                ...
                BOOST_TEST_MESSAGE("✓ Collection cancelled due to timeout in "
                                   << elapsed.count() << "ms: " << e.what());
        ...
        // Test 2: Manual cancellation using cancel_collection
        {
            ...
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                cancel_collection(futures_copy);
    ...
    // Test 3: Cancellation during active collection
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(active_futures),
                                 std::chrono::milliseconds(300));  // Medium timeout
        ...
        try {
            ...
            BOOST_TEST_MESSAGE("✓ Active collection completed with "
                               << results.size() << " results in " << elapsed.count() << "ms");
            ...
            BOOST_TEST_MESSAGE("✓ Active collection cancelled in " << elapsed.count()
                                                                   << "ms: " << e.what());
    ...
    // Test 4: Resource cleanup verification
    {
        ...
        raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
            cancel_collection(resource_futures);

        // Property: All resources should be cleaned up
        BOOST_CHECK(resource_futures.empty());
        BOOST_TEST_MESSAGE("✓ Resource cleanup verified: " << original_size
                                                           << " futures cleaned up");
    }
    ...
    // Test 5: Cancellation with different future types
    {
        ...
        raft_future_collector<kythira::request_vote_response<std::uint64_t>>::cancel_collection(
            vote_futures);
    ...
    // Test 6: Multiple concurrent cancellations
    {
        ...
        BOOST_TEST_MESSAGE("✓ Multiple concurrent cancellations: " << total_cancelled
                                                                   << " futures cleaned up");
    }
    ...
    // Test 7: Cancellation edge cases
    {
        ...
        raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
            cancel_collection(single_future);
    ...
    // Test 8: Cancellation doesn't affect completed futures
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(immediate_futures), test_timeout);

        try {
            auto results = std::move(collection_future).get();
            BOOST_TEST_MESSAGE("✓ Immediate collection completed with " << results.size()
                                                                        << " results");
