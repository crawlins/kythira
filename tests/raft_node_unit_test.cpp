...
BOOST_AUTO_TEST_CASE(submit_command_with_session_on_follower, * boost::unit_test::timeout(10)) {
    ...
    auto future = node.submit_command_with_session(42, 1, make_put_cmd("k", "v"), std::chrono::milliseconds{500});
    BOOST_CHECK_THROW(std::move(future).get(), std::exception);
...
BOOST_AUTO_TEST_CASE(single_node_multiple_commands, * boost::unit_test::timeout(10)) {
    ...
    for (auto [k, v] : std::vector<std::pair<std::string,std::string>>{{"a","1"},{"b","2"},{"c","3"}}) {
...
BOOST_AUTO_TEST_CASE(submit_command_with_session_commits, * boost::unit_test::timeout(10)) {
    ...
    auto future = node.submit_command_with_session(1, 1, make_put_cmd("k", "v"), std::chrono::milliseconds{2000});
    auto result = std::move(future).get();
...
BOOST_AUTO_TEST_CASE(three_node_command_replication, * boost::unit_test::timeout(30)) {
    ...
    auto r1 = std::move(node1.submit_command(make_put_cmd("a","1"), std::chrono::milliseconds{2000})).get();
    auto r2 = std::move(node2.submit_command(make_put_cmd("b","2"), std::chrono::milliseconds{2000})).get();
    auto r3 = std::move(node3.submit_command(make_put_cmd("c","3"), std::chrono::milliseconds{2000})).get();

    BOOST_CHECK(r1.empty()); BOOST_CHECK(r2.empty()); BOOST_CHECK(r3.empty());
