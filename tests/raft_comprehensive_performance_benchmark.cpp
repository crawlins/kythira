...
namespace {
    ...
    public:
        ...
        auto get_average_cpu() const -> double {
            ...
            double sum = std::accumulate(snapshots_.begin(), snapshots_.end(), 0.0,
                [](double acc, const auto& snap) {
                    return acc + snap.cpu_percent;
                });
...
BOOST_AUTO_TEST_CASE(benchmark_resource_memory_footprint, * boost::unit_test::timeout(180)) {
    ...
    for (auto size : state_sizes) {
        ...
        BOOST_TEST_MESSAGE("  Bytes per operation: " << std::fixed << std::setprecision(2)
                          << bytes_per_operation);
        BOOST_TEST_MESSAGE("  Memory efficiency: " 
                          << (bytes_per_operation < 100 ? "Excellent" :
                              bytes_per_operation < 500 ? "Good" :
                              bytes_per_operation < 1000 ? "Fair" : "Poor"));
...
BOOST_AUTO_TEST_CASE(benchmark_resource_memory_growth, * boost::unit_test::timeout(180)) {
    ...
    for (auto checkpoint : checkpoints) {
        ...
        BOOST_TEST_MESSAGE("Checkpoint " << current_ops << " ops: " 
                          << snapshot.size() << " bytes");
    }
    ...
    for (std::size_t i = 1; i < growth_data.size(); ++i) {
        ...
        BOOST_TEST_MESSAGE("  " << prev_ops << " -> " << curr_ops << " ops: "
                          << std::fixed << std::setprecision(2)
                          << growth_rate << " bytes/op growth rate");
    }
...
BOOST_AUTO_TEST_CASE(benchmark_regression_throughput_baseline, * boost::unit_test::timeout(120)) {
    ...
    BOOST_TEST_MESSAGE("  Measured: " << std::fixed << std::setprecision(2) 
                       << ops_per_second << " ops/sec");
...
BOOST_AUTO_TEST_CASE(benchmark_regression_stability, * boost::unit_test::timeout(180)) {
    ...
    BOOST_TEST_MESSAGE("\nStability Analysis:");
    BOOST_TEST_MESSAGE("  Throughput CV: " << std::fixed << std::setprecision(2) 
                       << throughput_cv << "%");
    ...
    constexpr double max_latency_cv = 70.0;  // Relaxed for nanosecond-precision measurements with system noise
