...
BOOST_AUTO_TEST_CASE(test_legacy_dtls_fields_still_select_dtls,
    ...
    // here (Requirement 5.4). *Where* that surfaces differs by build, and both
    // are correct:
    //
    //   with libnyoci     construction plans the DTLS channel and succeeds; the
    //                     certificate is only loaded by start(), and these paths
    //                     do not exist, so start() throws.
    //   without libnyoci  there is no DTLS to plan, so plan_security() refuses
    //                     at construction.
    //
    // The first version of this test asserted only the former and ran in both
    // builds, which passed locally and failed in CI -- where the backend is off
    // by default. Asserting the actual contract of each build is the fix.
    kythira::coap_server_config config;
