...
BOOST_AUTO_TEST_CASE(test_concept_satisfaction, *boost::unit_test::timeout(30)) {
    ...
    static_assert(
        kythira::state_machine<kythira::test_key_value_state_machine<std::uint64_t>, std::uint64_t>,
...
BOOST_AUTO_TEST_CASE(test_apply_method_signature, *boost::unit_test::timeout(30)) {
    ...
    auto command =
        kythira::test_key_value_state_machine<>::make_put_command(test_key_foo, test_value_hello);
...
BOOST_AUTO_TEST_CASE(test_multiple_operations, *boost::unit_test::timeout(30)) {
    ...
    auto put2 =
        kythira::test_key_value_state_machine<>::make_put_command(test_key_bar, test_value_world);
    sm.apply(put2, test_index_2);

    auto put3 =
        kythira::test_key_value_state_machine<>::make_put_command(test_key_baz, test_value_test);
...
BOOST_AUTO_TEST_CASE(test_update_existing_key, *boost::unit_test::timeout(30)) {
    ...
    // PUT updated value
    auto put2 =
        kythira::test_key_value_state_machine<>::make_put_command(test_key_foo, test_value_updated);
    sm.apply(put2, test_index_2);
...
BOOST_AUTO_TEST_CASE(test_restore_from_invalid_snapshot, *boost::unit_test::timeout(30)) {
    ...
    BOOST_CHECK_THROW(sm.restore_from_snapshot(invalid_snapshot, test_index_1),
