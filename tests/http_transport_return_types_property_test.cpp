...
using test_transport_types = kythira::simple_http_transport_types<
    kythira::json_rpc_serializer<std::vector<std::byte>>, kythira::noop_metrics,
    kythira::noop_metrics  // Using noop_metrics as executor placeholder since folly is not
                           // available
    >;
}
...
BOOST_AUTO_TEST_CASE(property_http_transport_return_types, *boost::unit_test::timeout(30)) {
    ...
    static_assert(kythira::future<MockFuture<kythira::request_vote_response<>>,
                                  kythira::request_vote_response<>>,
                  "MockFuture should satisfy future concept for request_vote_response");
    static_assert(kythira::future<MockFuture<kythira::append_entries_response<>>,
                                  kythira::append_entries_response<>>,
                  "MockFuture should satisfy future concept for append_entries_response");
    static_assert(kythira::future<MockFuture<kythira::install_snapshot_response<>>,
...
BOOST_AUTO_TEST_CASE(property_http_transport_future_design_pattern,
...
// Property: Verify that the conversion goal is to replace hardcoded future types with template
// parameters
BOOST_AUTO_TEST_CASE(property_http_transport_conversion_goal, *boost::unit_test::timeout(30)) {
    ...
    // Test that the template template parameter approach allows for different future
    // implementations This is the key architectural decision: flexible future types via template
    // template parameters
...
BOOST_AUTO_TEST_CASE(test_transport_types_concept_conformance, *boost::unit_test::timeout(30)) {
    ...
    static_assert(std::is_same_v<typename test_transport_types::template future_template<
                                     kythira::request_vote_response<>>,
    ...
    static_assert(
        std::is_same_v<typename test_transport_types::template future_template<
                           kythira::append_entries_response<>>,
    ...
    static_assert(
        std::is_same_v<typename test_transport_types::metrics_type, kythira::noop_metrics>,
    ...
    static_assert(kythira::future<typename test_transport_types::template future_template<
                                      kythira::request_vote_response<>>,
                                  kythira::request_vote_response<>>,
...
BOOST_AUTO_TEST_CASE(test_template_template_parameter_future_type_correctness,
                     *boost::unit_test::timeout(30)) {
    ...
    static_assert(
        std::is_same_v<typename test_transport_types::template future_template<
                           kythira::install_snapshot_response<>>,
                       network_simulator::SimpleFuture<kythira::install_snapshot_response<>>>,
        "future_template<install_snapshot_response> must be "
    ...
    static_assert(std::is_same_v<decltype(vote_future),
                                 typename test_transport_types::template future_template<
                                     kythira::request_vote_response<>>>,
...
BOOST_AUTO_TEST_CASE(test_alternative_future_implementations, *boost::unit_test::timeout(30)) {
    // Test that the template template parameter approach allows for different future
    // implementations Note: This test demonstrates the concept, but std::future doesn't satisfy our
    // future concept which is designed for folly::Future-like interfaces
