...
public:
    explicit ReplicationManager(std::size_t cluster_size) : _cluster_size(cluster_size) {
    ...
    std::size_t get_slow_followers_count() const { return _slow_followers.size(); }

    // Get unresponsive followers count
    std::size_t get_unresponsive_followers_count() const { return _unresponsive_followers.size(); }
 ...
 *
 * Property: For any slow follower responses, the system continues replication without blocking
 * other operations.
 * **Validates: Requirements 6.3**
 */
BOOST_AUTO_TEST_CASE(raft_non_blocking_slow_followers_property_test,
    ...
    std::uniform_int_distribution<int> slow_follower_rate_dist(
        10, 40);  // percentage of followers that are slow
    std::uniform_int_distribution<int> unresponsive_follower_rate_dist(
        0, 20);  // percentage that are unresponsive
    std::uniform_int_distribution<int> ack_rate_dist(
        70, 100);  // acknowledgment rate for responsive followers
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        BOOST_TEST_MESSAGE("Responsive count: "
                           << responsive_count << ", Majority needed: " << majority_needed
        ...
        for (LogIndex log_index = 1; log_index <= entry_count; ++log_index) {
            ...
            // Property 2: Commit index should advance based on responsive majority, not total
            // cluster size
    ...
    // Test case: All followers are slow but still responsive
    {
        ...
        BOOST_CHECK_EQUAL(slow_manager.get_responsive_count(),
                          5);  // All nodes are responsive (slow != unresponsive)
    ...
    // Test case: Some followers are unresponsive, but majority remains responsive
    {
        ...
        BOOST_CHECK_EQUAL(unresponsive_manager.get_responsive_count(),
                          5);  // 7 - 2 unresponsive = 5
    ...
    // Test case: Too many unresponsive followers, cannot continue
    {
        ...
        blocked_manager.record_acknowledgment(
            1, 999);  // Any follower ID (won't affect single responsive node)
    ...
    // Test case: Follower state transitions
    {
        ...
        BOOST_CHECK_EQUAL(transition_manager.get_follower_state(test_follower),
                          FollowerState::responsive);
    ...
    // Test case: Mixed follower states with sequential entries
    {
        ...
        // All entries should be committed (leader + 3 responsive followers = 4, which is majority
        // of 6)
