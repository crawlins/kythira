...
struct FollyInitFixture {
    FollyInitFixture() {
        int argc = 1;
        char* argv_data[] = {const_cast<char*>("raft_application_success_handling_property_test"),
                             nullptr};
        char** argv = argv_data;
...
public:
    ...
    auto record_application_success(std::uint64_t log_index, const std::vector<std::byte>& command,
                                    std::uint64_t new_applied_index) -> void {
        std::lock_guard<std::mutex> lock(_mutex);
        _applications.push_back(
            {log_index, command, true, std::chrono::steady_clock::now(), new_applied_index});
    }
    ...
    auto verify_all_successful() const -> bool {
        ...
        return std::all_of(_applications.begin(), _applications.end(),
                           [](const ApplicationResult& result) { return result.success; });
    }
...
BOOST_AUTO_TEST_CASE(property_application_success_handling, *boost::unit_test::timeout(120)) {
    ...
    for (std::size_t iteration = 0; iteration < property_test_iterations; ++iteration) {
        ...
        BOOST_CHECK_MESSAGE(
            tracker.verify_applied_index_progression(),
...
BOOST_AUTO_TEST_CASE(property_future_fulfillment_simulation, *boost::unit_test::timeout(90)) {
    ...
    for (std::size_t iteration = 0; iteration < property_test_iterations; ++iteration) {
        ...
        BOOST_CHECK_MESSAGE(
            tracker.get_application_count() == pending_count,
