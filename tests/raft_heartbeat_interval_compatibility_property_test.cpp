...
constexpr double recommended_ratio =
    3.0;  // Election timeout should be at least 3x heartbeat interval
}
 ...
 *
 * Property: When configuring heartbeat intervals, the system ensures the interval is compatible
 * with election timeouts.
 * **Validates: Requirements 9.3**
 */
BOOST_AUTO_TEST_CASE(raft_heartbeat_interval_compatibility_property_test,
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        std::uniform_int_distribution<int> heartbeat_dist(min_heartbeat.count(),
                                                          max_heartbeat.count());
        std::uniform_int_distribution<int> election_dist(min_election.count(),
                                                         max_election.count());
        ...
        BOOST_TEST_MESSAGE("Testing compatibility - Heartbeat: "
        ...
        if (ratio < recommended_ratio) {
            ...
            bool has_compatibility_error = std::any_of(
            ...
            // Property: When heartbeat interval is appropriately sized, no compatibility error
            // should occur
            bool has_compatibility_error = std::any_of(
                validation_errors.begin(), validation_errors.end(), [](const std::string& error) {
                    return error.find("heartbeat_interval") != std::string::npos &&
    ...
    // Test 2: Explicit compatibility violations
    {
        ...
        std::vector<std::pair<std::chrono::milliseconds, std::chrono::milliseconds>>
            incompatible_pairs = {
    ...
    // Test 4: Edge case ratios
    {
        ...
        bool has_boundary_error = std::any_of(
            boundary_errors.begin(), boundary_errors.end(), [](const std::string& error) {
                return error.find("heartbeat_interval") != std::string::npos &&
                       error.find("election_timeout") != std::string::npos;
            });
        ...
        below_boundary_config._election_timeout_min =
            std::chrono::milliseconds{299};  // Just below 3x
        ...
        bool has_below_boundary_error =
            std::any_of(below_boundary_errors.begin(), below_boundary_errors.end(),
    ...
    // Test 5: Very small and very large values
    {
        ...
        bool has_small_compatibility_error =
            std::any_of(small_errors.begin(), small_errors.end(), [](const std::string& error) {
                return error.find("heartbeat_interval") != std::string::npos &&
                       error.find("election_timeout") != std::string::npos;
            });
        ...
        bool has_large_compatibility_error =
            std::any_of(large_errors.begin(), large_errors.end(), [](const std::string& error) {
                return error.find("heartbeat_interval") != std::string::npos &&
                       error.find("election_timeout") != std::string::npos;
            });
    ...
    // Test 6: Practical Raft timing scenarios
    {
        ...
        bool has_fast_compatibility_error =
            std::any_of(fast_errors.begin(), fast_errors.end(), [](const std::string& error) {
                return error.find("heartbeat_interval") != std::string::npos &&
                       error.find("election_timeout") != std::string::npos;
            });
        ...
        bool has_slow_compatibility_error =
            std::any_of(slow_errors.begin(), slow_errors.end(), [](const std::string& error) {
                return error.find("heartbeat_interval") != std::string::npos &&
                       error.find("election_timeout") != std::string::npos;
            });
        ...
        bool has_conservative_compatibility_error = std::any_of(
            conservative_errors.begin(), conservative_errors.end(), [](const std::string& error) {
                return error.find("heartbeat_interval") != std::string::npos &&
                       error.find("election_timeout") != std::string::npos;
            });
        ...
        std::vector<std::pair<std::chrono::milliseconds, std::chrono::milliseconds>>
            election_ranges = {
                ...
                {std::chrono::milliseconds{1000},
                 std::chrono::milliseconds{1500}},  // Min ratio: 10.0
                {std::chrono::milliseconds{200},
                 std::chrono::milliseconds{300}},  // Min ratio: 2.0 (should fail)
                {std::chrono::milliseconds{250},
                 std::chrono::milliseconds{350}}  // Min ratio: 2.5 (should fail)
            };
    ...
    // Test 9: Compatibility error message validation
    {
        ...
        for (const auto& error : validation_errors) {
            if (error.find("heartbeat_interval") != std::string::npos &&
                error.find("election_timeout") != std::string::npos &&
                (error.find("less than") != std::string::npos ||
                 error.find("prevent") != std::string::npos)) {
