...
public:
    ...
    // Move constructor
    test_logger(test_logger&& other) noexcept : _entries(std::move(other._entries)) {}
    ...
    [[nodiscard]] auto has_log_with_message(const std::string& message) const -> bool {
        std::lock_guard<std::mutex> lock(_mutex);
        return std::any_of(_entries.begin(), _entries.end(), [&message](const log_entry& entry) {
    ...
    [[nodiscard]] auto has_log_with_level(kythira::log_level level) const -> bool {
        ...
        return std::any_of(_entries.begin(), _entries.end(),
                           [level](const log_entry& entry) { return entry.level == level; });
    }

    [[nodiscard]] auto has_log_with_key_value(const std::string& key,
                                              const std::string& value) const -> bool {
        std::lock_guard<std::mutex> lock(_mutex);
        return std::any_of(
            _entries.begin(), _entries.end(), [&key, &value](const log_entry& entry) {
        ...
        try {
            ...
            // the logging infrastructure is properly integrated
            {
                auto client = kythira::coap_client<test_transport_types>{
                    std::move(endpoints), std::move(config), std::move(metrics)};
...
BOOST_AUTO_TEST_CASE(test_coap_server_lifecycle_logging, *boost::unit_test::timeout(60)) {
    ...
    BOOST_TEST_MESSAGE("CoAP server lifecycle with logging: " << successful_operations << "/"
...
BOOST_AUTO_TEST_CASE(test_coap_error_logging, *boost::unit_test::timeout(30)) {
    ...
    for (const auto& endpoint : test_endpoints) {
        try {
            ...
            // Create CoAP client - this tests that logging infrastructure can handle various
            // endpoints
            auto client = kythira::coap_client<test_transport_types>{
                std::move(endpoints), std::move(config), std::move(metrics)};
    ...
    // All tests should succeed since we're testing the logging infrastructure, not endpoint
    // validity
