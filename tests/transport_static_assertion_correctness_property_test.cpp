...
BOOST_AUTO_TEST_CASE(property_static_assertion_correctness, * boost::unit_test::timeout(60)) {
    ...
    using http_client_type = kythira::cpp_httplib_client<future_type, test_serializer, test_metrics>;
    using http_server_type = kythira::cpp_httplib_server<future_type, test_serializer, test_metrics>;
    
    static_assert(kythira::network_client<http_client_type, future_type>,
                 "HTTP client must satisfy network_client concept with correct template parameters");
    
    static_assert(kythira::network_server<http_server_type, future_type>,
                 "HTTP server must satisfy network_server concept with correct template parameters");
    
    // Test 2: Verify simulator network types satisfy network concepts
    using simulator_client_type = kythira::simulator_network_client<future_type, test_serializer, std::vector<std::byte>>;
    using simulator_server_type = kythira::simulator_network_server<future_type, test_serializer, std::vector<std::byte>>;
    
    static_assert(kythira::network_client<simulator_client_type, future_type>,
                 "Simulator client must satisfy network_client concept with correct template parameters");
    
    static_assert(kythira::network_server<simulator_server_type, future_type>,
                 "Simulator server must satisfy network_server concept with correct template parameters");
    ...
    static_assert(kythira::network_client<coap_client_type, future_type>,
                 "CoAP client must satisfy network_client concept with correct template parameters");
    
    static_assert(kythira::network_server<coap_server_type, future_type>,
                 "CoAP server must satisfy network_server concept with correct template parameters");
...
BOOST_AUTO_TEST_CASE(test_static_assertion_template_parameter_count, * boost::unit_test::timeout(30)) {
    // Test that network concepts require exactly 2 template parameters
    ...
    using sim_client = kythira::simulator_network_client<rv_future_type, test_serializer, std::vector<std::byte>>;
    using sim_server = kythira::simulator_network_server<rv_future_type, test_serializer, std::vector<std::byte>>;
    
    static_assert(kythira::network_client<sim_client, rv_future_type>,
                 "Simulator client must satisfy network_client concept with 2 template parameters");
    
    static_assert(kythira::network_server<sim_server, rv_future_type>,
                 "Simulator server must satisfy network_server concept with 2 template parameters");
