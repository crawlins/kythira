 ...
 *
 * Property: For any heartbeat RPC failure, the system retries with exponential backoff up to
 * configured limits.
...
BOOST_AUTO_TEST_CASE(raft_heartbeat_retry_backoff_property_test, *boost::unit_test::timeout(180)) {
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        typename error_handler<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
            retry_policy heartbeat_policy{.initial_delay = base_delay,
                                          .max_delay = max_delay,
        ...
        auto heartbeat_operation = [&attempt_count, failures_before_success, &attempt_times]()
            ...
            if (current_attempt <= failures_before_success) {
                ...
                std::vector<std::string> failure_messages = {
                    "Network timeout occurred", "Connection refused by target",
                    "Network is unreachable", "Temporary failure, try again"};
        ...
        try {
            ...
            BOOST_TEST_MESSAGE("✓ Operation succeeded after "
            ...
            if (attempt_times.size() > 1) {
                for (std::size_t i = 1; i < attempt_times.size(); ++i) {
                    ...
                    // Calculate expected delay for this retry (with some tolerance for jitter and
                    // timing)
    ...
    // Test 1: Max attempts exceeded
    {
        ...
        typename error_handler<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
            retry_policy strict_policy{.initial_delay = std::chrono::milliseconds{50},
        ...
        auto start_time = std::chrono::steady_clock::now();
        BOOST_CHECK_THROW(handler.execute_with_retry("heartbeat", always_fail_operation).get(),
                          std::exception);
        auto end_time = std::chrono::steady_clock::now();
        ...
        BOOST_TEST_MESSAGE("✓ Max attempts test: " << attempt_count.load() << " attempts in "
                                                   << elapsed.count() << "ms");
    }
    ...
    // Test 3: Jitter effectiveness
    {
        ...
        typename error_handler<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
            retry_policy jitter_policy{.initial_delay = std::chrono::milliseconds{100},
        ...
        if (total_times.size() > 1) {
            ...
            BOOST_TEST_MESSAGE("Time variation: " << time_variation.count()
                                                  << "ms (min=" << min_time.count()
                                                  << "ms, max=" << max_time.count() << "ms)");
