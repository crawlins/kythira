 ...
 *
 * Property: For any log entry replication, commit index advances only when majority acknowledgment
 * is received.
 * **Validates: Requirements 2.3**
 */
BOOST_AUTO_TEST_CASE(raft_replication_majority_acknowledgment_property_test,
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(replication_futures), test_timeout);

        try {
            auto results = std::move(collection_future).get();

            // Property: collect_majority should return a majority of responses when possible
            BOOST_TEST_MESSAGE("✓ Replication collection returned " << results.size()
                                                                    << " responses");
            ...
            const std::size_t total_successful_acks =
                successful_in_results + 1;  // +1 for leader's self-acknowledgment
            const bool should_commit = total_successful_acks >= majority_count;

            if (should_commit) {
                BOOST_TEST_MESSAGE("✓ Entry should be committed with "
                                   << total_successful_acks
                                   << " acknowledgments (including leader)");
            } else {
                BOOST_TEST_MESSAGE("✓ Entry should NOT be committed with "
                                   << total_successful_acks
                                   << " acknowledgments (including leader)");
            }
    ...
    // Test all successful replication scenario
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(all_success_futures), test_timeout);
    ...
    // Test mixed success/failure scenario
    {
        ...
        for (std::size_t i = 0; i < mixed_followers; ++i) {
            ...
            kythira::append_entries_response<std::uint64_t, std::uint64_t> response{
                ...
                success,  // success
                success ? std::nullopt
                        : std::make_optional(target_index),  // conflict_index only for failures
                success ? std::nullopt
        ...
        BOOST_TEST_MESSAGE("✓ Mixed success/failure scenario: "
                           << successful_count << " successful acknowledgments out of "
