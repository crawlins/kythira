...
public:
    ...
    auto trace(std::string_view message) -> void {
        log(raft::log_level::trace, message);
    }
    
    auto debug(std::string_view message) -> void {
        log(raft::log_level::debug, message);
    }
    
    auto info(std::string_view message) -> void {
        log(raft::log_level::info, message);
    }
    
    auto warning(std::string_view message) -> void {
        log(raft::log_level::warning, message);
    }
    
    auto error(std::string_view message) -> void {
        log(raft::log_level::error, message);
    }
    
    auto critical(std::string_view message) -> void {
        log(raft::log_level::critical, message);
    }

private:
    auto level_to_string(raft::log_level level) -> std::string_view {
        switch (level) {
            case raft::log_level::trace: return "TRACE";
            case raft::log_level::debug: return "DEBUG";
            case raft::log_level::info: return "INFO";
            case raft::log_level::warning: return "WARNING";
            case raft::log_level::error: return "ERROR";
            case raft::log_level::critical: return "CRITICAL";
        }
        return "UNKNOWN";
    }
};
...
static_assert(raft::diagnostic_logger<console_logger>, 
...
auto main() -> int {
    ...
    static_assert(static_cast<std::uint8_t>(raft::log_level::trace) == 0);
    static_assert(static_cast<std::uint8_t>(raft::log_level::debug) == 1);
    static_assert(static_cast<std::uint8_t>(raft::log_level::info) == 2);
    static_assert(static_cast<std::uint8_t>(raft::log_level::warning) == 3);
    static_assert(static_cast<std::uint8_t>(raft::log_level::error) == 4);
    static_assert(static_cast<std::uint8_t>(raft::log_level::critical) == 5);
