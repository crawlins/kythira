...
BOOST_AUTO_TEST_CASE(property_handler_invocation_for_all_rpcs) {
    ...
    mock_server.Post(
        "/v1/raft/request_vote", [&](const httplib::Request& req, httplib::Response& res) {
            request_vote_count++;
    ...
    mock_server.Post(
        "/v1/raft/append_entries", [&](const httplib::Request& req, httplib::Response& res) {
    ...
    std::thread server_thread([&]() { mock_server.listen("127.0.0.1", server_port); });
    ...
    try {
        ...
        for (std::size_t i = 1; i <= 5; ++i) {
            // Test RequestVote
            {
                ...
                BOOST_TEST_MESSAGE("RequestVote - Content-Length: "
                                   << content_length_value << ", body size: " << actual_body_size);
            ...
            // Test AppendEntries
            {
                ...
                // The Content-Length should be reasonable (allowing for minor encoding differences)
                BOOST_TEST_MESSAGE("AppendEntries - Content-Length: "
                                   << content_length_value << ", body size: " << actual_body_size);
                BOOST_TEST(content_length_value > 0);
                BOOST_TEST(content_length_value >= actual_body_size - 5);
                BOOST_TEST(content_length_value <= actual_body_size + 25);
            ...
            // Test InstallSnapshot
            {
                ...
                // The Content-Length should be reasonable (allowing for minor encoding differences)
                BOOST_TEST_MESSAGE("InstallSnapshot - Content-Length: "
                                   << content_length_value << ", body size: " << actual_body_size);
                BOOST_TEST(content_length_value > 0);
                BOOST_TEST(content_length_value >= actual_body_size - 5);
                BOOST_TEST(content_length_value <= actual_body_size + 25);
