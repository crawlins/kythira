...
public:
    ...
    [[nodiscard]] auto get_state() const -> std::vector<std::byte> {
        ...
        const auto* index_bytes = reinterpret_cast<const std::byte*>(&_last_applied_index);
    ...
    auto restore_from_snapshot(const std::vector<std::byte>& snapshot) -> void {
        ...
        if (snapshot.size() < sizeof(_last_applied_index)) {
            return;
        }
    ...
    [[nodiscard]] auto get_last_applied_index() const -> std::uint64_t {
