 ...
 *
 * Property: For any future collection with timeouts, individual timeouts are handled without
 * blocking other operations.
 * **Validates: Requirements 2.4**
 */
BOOST_AUTO_TEST_CASE(raft_timeout_handling_collections_property_test,
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        BOOST_TEST_MESSAGE("Testing with " << future_count
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(mixed_futures), short_timeout);
    ...
    // Test 1: All futures timeout
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(timeout_futures), short_timeout);
        ...
        BOOST_TEST_MESSAGE("✓ All-timeout scenario handled correctly in " << elapsed.count()
                                                                          << "ms");
    }
    ...
    // Test 2: Mix of fast and slow responses
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(mixed_futures), medium_timeout);
        ...
        try {
            ...
            BOOST_TEST_MESSAGE("✓ Mixed scenario completed with "
    ...
    // Test 3: Timeout with concurrent operations
    {
        ...
        for (std::size_t collection = 0; collection < 3; ++collection) {
            ...
            auto collection_future = raft_future_collector<kythira::append_entries_response<
                std::uint64_t, std::uint64_t>>::collect_majority(std::move(futures),
                                                                 medium_timeout);
        ...
        for (auto& collection : concurrent_collections) {
            try {
                ...
                BOOST_TEST_MESSAGE("Concurrent collection succeeded with " << results.size()
                                                                           << " results");
            } catch (const std::exception& e) {
        ...
        BOOST_TEST_MESSAGE("✓ " << successful_collections
                                << " out of 3 concurrent collections completed in "
                                << elapsed.count() << "ms");
    ...
    // Test 4: Timeout precision
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(precise_futures), precise_timeout);
        ...
        BOOST_CHECK_GE(elapsed.count(),
                       precise_timeout.count() - 50);  // Allow some early completion
        BOOST_CHECK_LE(elapsed.count(), precise_timeout.count() + 300);  // Allow some overhead

        BOOST_TEST_MESSAGE("✓ Timeout precision test: expected "
                           << precise_timeout.count() << "ms, actual " << elapsed.count() << "ms");
    }
