        ...
        try {
            ...
            // Test AppendEntries future resolution
            {
                ...
                try {
                    ...
                    BOOST_TEST_MESSAGE("AppendEntries future resolved with success at iteration "
                                       << i);
                } catch (const std::exception& e) {
                    future_resolved = true;
                    BOOST_TEST_MESSAGE("AppendEntries future resolved with error at iteration "
                                       << i << ": " << e.what());
                }
            ...
            // Test InstallSnapshot future resolution
            {
                ...
                try {
                    auto result = std::move(future).get();
                    future_resolved = true;
                    BOOST_TEST_MESSAGE("InstallSnapshot future resolved with success at iteration "
                                       << i);
                } catch (const std::exception& e) {
                    future_resolved = true;
                    BOOST_TEST_MESSAGE("InstallSnapshot future resolved with error at iteration "
                                       << i << ": " << e.what());
                }
            ...
            BOOST_TEST_MESSAGE("Exception during interface validation test " << i << ": "
                                                                             << e.what());
        }
    }

    BOOST_TEST_MESSAGE("Interface validation property: " << successful_iterations << "/"
...
BOOST_AUTO_TEST_CASE(test_concurrent_future_resolution, *boost::unit_test::timeout(60)) {
    ...
    BOOST_TEST_MESSAGE("Concurrent interface validation test passed: " << successful_requests
                                                                       << "/5 requests validated");
}
