 ...
 *
 * Property: For any failed heartbeat collection during read, the read request is rejected with
 * leadership error.
...
BOOST_AUTO_TEST_CASE(raft_failed_read_rejection_property_test, *boost::unit_test::timeout(120)) {
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        BOOST_TEST_MESSAGE("Testing cluster size: "
                           << cluster_size << ", majority needed: " << majority_count
        ...
        if (scenario == 0) {
            ...
            const std::size_t max_successes =
                (required_successful_followers > 0) ? required_successful_followers - 1 : 0;

            for (std::size_t i = 0; i < follower_count; ++i) {
                const int delay_ms = delay_dist(gen);
                const bool will_succeed =
                    (successful_responses < max_successes) && (gen() % 3 == 0);  // Low success rate
            ...
            BOOST_TEST_MESSAGE("Simulated " << successful_responses
                                            << " successful responses (insufficient for majority)");
            ...
            auto collection_future = raft_future_collector<kythira::append_entries_response<
                std::uint64_t, std::uint64_t>>::collect_majority(std::move(heartbeat_futures),
                                                                 test_timeout);
            ...
            try {
                ...
                BOOST_TEST_MESSAGE(
                    "✓ Heartbeat collection failed, read correctly rejected: " << e.what());
            }
            ...
            auto collection_future = raft_future_collector<kythira::append_entries_response<
                std::uint64_t, std::uint64_t>>::collect_majority(std::move(timeout_futures),
            ...
            const std::size_t max_successes =
                (required_successful_followers > 1) ? required_successful_followers - 1 : 0;
            ...
            BOOST_TEST_MESSAGE("Simulated " << successful_responses << " successful, "
                                            << failed_responses << " failed, " << timeout_responses
                                            << " timeout responses");

            // Test that mixed failures with insufficient majority reject read
            auto collection_future = raft_future_collector<kythira::append_entries_response<
                std::uint64_t, std::uint64_t>>::collect_majority(std::move(mixed_futures),
                                                                 test_timeout);
            ...
            try {
                ...
                // Property: If insufficient majority, read should be rejected
                if (total_success < majority_count) {
                    BOOST_TEST_MESSAGE("✓ Mixed failures with insufficient majority ("
                                       << total_success << "/" << majority_count
                                       << "), read correctly rejected");
                } else {
                ...
                // Property: Collection failure should cause read rejection
                BOOST_TEST_MESSAGE(
                    "✓ Mixed failures correctly caused read rejection: " << e.what());
            }
        }
    }
    ...
    // Test with all failed responses (same term, but all failed)
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(all_failed_futures), test_timeout);
    ...
    // Test with very short timeout (should cause timeout failures)
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(slow_futures),
        ...
        try {
            ...
            BOOST_TEST_MESSAGE(
                "✓ Very short timeout correctly caused read rejection: " << e.what());
        }
    }
    ...
    // Test with empty futures (should fail immediately)
    {
        ...
        auto collection_future =
            raft_future_collector<kythira::append_entries_response<std::uint64_t, std::uint64_t>>::
                collect_majority(std::move(empty_futures), test_timeout);
        ...
        try {
            ...
            BOOST_TEST_MESSAGE(
                "✓ Empty futures correctly caused immediate read rejection: " << e.what());
        }
    }
