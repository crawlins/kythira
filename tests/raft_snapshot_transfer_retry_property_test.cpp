 ...
 *
 * Property: For any InstallSnapshot RPC failure, the system retries snapshot transfer with proper
 * error recovery.
...
BOOST_AUTO_TEST_CASE(raft_snapshot_transfer_retry_property_test, *boost::unit_test::timeout(300)) {
    ...
    std::uniform_int_distribution<int> failure_count_dist(1,
                                                          4);  // Number of failures before success
    ...
    for (std::size_t iteration = 0; iteration < test_iterations; ++iteration) {
        ...
        typename error_handler<kythira::install_snapshot_response<std::uint64_t>>::retry_policy
            snapshot_policy{.initial_delay = base_delay,
                            .max_delay = max_delay,
        ...
        try {
            auto result =
                handler.execute_with_retry("install_snapshot", snapshot_transfer_operation).get();
            ...
            BOOST_TEST_MESSAGE("✓ Snapshot transfer succeeded after "
                               << attempt_count.load() << " attempts in " << total_elapsed.count()
                               << "ms, transferred " << bytes_transferred.load() << " bytes");
            ...
            for (const auto& failure_mode : failure_modes_encountered) {
                ...
                if (failure_mode.find("timeout") != std::string::npos ||
                    ...
                    BOOST_TEST_MESSAGE(
                        "Disk full error classification: " << classification.should_retry);
                }
            }
            ...
            BOOST_TEST_MESSAGE("Snapshot transfer failed after "
                               << attempt_count.load() << " attempts in " << total_elapsed.count()
                               << "ms: " << e.what());
    ...
    // Test 1: Large snapshot with progressive backoff
    {
        ...
        try {
            auto result =
                handler.execute_with_retry("install_snapshot", large_snapshot_operation).get();
            ...
            if (attempt_times.size() >= 4) {
                ...
                BOOST_TEST_MESSAGE("Large snapshot delays: " << delay1.count() << "ms, "
                                                             << delay2.count() << "ms, "
                                                             << delay3.count() << "ms");
    ...
    // Test 2: Snapshot corruption detection
    {
        ...
        try {
            ...
            auto classification =
                handler.classify_error(std::runtime_error("Snapshot checksum validation failed"));

            // Checksum failures are typically not retryable
            if (classification.should_retry) {
                BOOST_TEST_MESSAGE(
                    "Note: Checksum failure classified as retryable - this may be acceptable "
    ...
    // Test 3: Different snapshot transfer error types
    {
        ...
        for (const auto& [error_msg, expected_retry] : snapshot_error_scenarios) {
            BOOST_TEST_MESSAGE("Testing snapshot error: " << error_msg << " (expected_retry="
                                                          << expected_retry << ")");
            ...
            try {
                ...
                if (expected_retry) {
                    if (classification.should_retry) {
                        ...
                        BOOST_TEST_MESSAGE("✓ Retryable snapshot error made "
    ...
    // Test 4: Snapshot transfer timeout progression
    {
        ...
        typename error_handler<kythira::install_snapshot_response<std::uint64_t>>::retry_policy
            timeout_policy{.initial_delay = std::chrono::milliseconds{50},  // Reduced from 100ms
        ...
        try {
            auto result =
                handler.execute_with_retry("install_snapshot", timeout_progression_operation).get();
            ...
            if (attempt_times.size() >= 5) {
                ...
                BOOST_TEST_MESSAGE("Timeout progression delays: "
                                   << delays[0].count() << "ms, " << delays[1].count() << "ms, "
                                   << delays[2].count() << "ms, " << delays[3].count() << "ms");
