...
public:
    explicit mock_async_network_client(
        std::uint64_t node_id, bool simulate_failures = false,
        std::chrono::milliseconds latency = std::chrono::milliseconds{10})
        : _node_id(node_id), _simulate_failures(simulate_failures), _simulated_latency(latency) {}
            ...
            .thenValue([this, target_node, term]() {
                if (_simulate_failures && target_node == follower_node_2_id) {
                    ...
                    throw std::runtime_error(
                        std::format("Network failure to node {}", target_node));
                }

                // Return successful heartbeat response
                return kythira::append_entries_response<>{._term = term,
                                                          ._success = true,
        ...
        return kythira::FutureFactory::makeFuture()
            ...
            .thenValue([this, target_node, term, candidate_id]() {
                if (_simulate_failures && target_node == follower_node_2_id) {
                    ...
                    throw std::runtime_error(
                        std::format("Vote request failed to node {}", target_node));
                }

                // Grant vote (simplified logic)
                return kythira::request_vote_response<>{._term = term, ._vote_granted = true};
            });
    }
        ...
        return kythira::FutureFactory::makeFuture()
            ...
            .thenValue([this, target_node, term, entry_data]() {
                if (_simulate_failures && target_node == follower_node_2_id) {
                    ...
                    throw std::runtime_error(
                        std::format("Replication failed to node {}", target_node));
                }
...
auto test_heartbeat_collection() -> bool {
    ...
    try {
        ...
        auto heartbeat_1 = network_client.send_heartbeat(follower_node_1_id,
                                                         1,  // term
                                                         rpc_timeout);
        heartbeat_futures.push_back(std::move(heartbeat_1));

        // Send heartbeat to follower 2
        auto heartbeat_2 = network_client.send_heartbeat(follower_node_2_id,
                                                         1,  // term
                                                         rpc_timeout);
        heartbeat_futures.push_back(std::move(heartbeat_2));

        // Use future collector to wait for majority response
        auto majority_future =
            kythira::raft_future_collector<kythira::append_entries_response<>>::collect_majority(
                std::move(heartbeat_futures), collection_timeout);
...
auto test_election_vote_collection() -> bool {
    ...
    try {
        ...
        auto network_client =
            mock_async_network_client{follower_node_1_id};  // Node 2 becomes candidate
        ...
        auto vote_request_3 = network_client.send_vote_request(follower_node_2_id,
                                                               2,                   // new term
                                                               follower_node_1_id,  // candidate id
                                                               rpc_timeout);
        ...
        auto vote_collection_future =
            kythira::raft_future_collector<kythira::request_vote_response<>>::collect_majority(
                std::move(vote_futures), collection_timeout);
...
auto test_future_collection_timeout_handling() -> bool {
    ...
    try {
        ...
        auto operation_2 = network_client.send_heartbeat(follower_node_2_id,
                                                         1,  // term
                                                         rpc_timeout);
        operation_futures.push_back(std::move(operation_2));

        // Use collect_all_with_timeout to handle partial failures
        auto timeout_future = kythira::raft_future_collector<kythira::append_entries_response<>>::
            collect_all_with_timeout(
                std::move(operation_futures), std::chrono::milliseconds{500}  // short timeout
            );
...
auto test_future_collection_cancellation() -> bool {
    ...
    try {
        ...
        for (std::uint64_t i = 0; i < 3; ++i) {
            auto future =
                network_client.send_heartbeat(follower_node_1_id + i,
                                              1,                               // term
        ...
        kythira::raft_future_collector<kythira::append_entries_response<>>::cancel_collection(
            futures_to_cancel);
...
auto test_commit_waiting_mechanism() -> bool {
    ...
    try {
        ...
        commit_waiter.notify_committed_and_applied(
            log_index, [&test_result](std::uint64_t) { return test_result; });
