...
echo
echo "  REAL_CLOUD_TESTS_AZURE_OBJECT_PERSISTENCE_ENABLED=true"
echo
echo "NOT done by this script, and still required before CI can use it:"
echo "  * 'Storage Blob Data Contributor' for the CI federated identity"
echo "    (AZURE_CI_CLIENT_ID), scoped to the CONTAINER. Run:"
echo "      scripts/ci-cloud-credentials/azure/provision-federated-identity.sh \\"
echo "          --github-org ORG --github-repo REPO \\"
echo "          --subscription-id SUB --resource-group ${RESOURCE_GROUP} \\"
echo "          --bundles <every bundle you want>,object-persistence \\"
echo "          --storage-account ${ACCOUNT} --storage-container ${CONTAINER}"
